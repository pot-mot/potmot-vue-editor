import { c as V, z as gt, A as mt, s as xt, g as _t, b as kt, a as bt, m as vt, B as wt, h as H, i as Tt } from "./index-14ad5708.js";
import { d as $t, f as St, a as Mt, g as ot } from "./svgDrawCommon-f26cad39-9c0a5ebd.js";
...
var Z = function() {
  var t = function(_, r, a, h) {
    for (a = a || {}, h = _.length; h--; a[_[h]] = r)
      ;
    return a;
  }, e = [1, 2], s = [1, 5], n = [6, 9, 11, 17, 18, 20, 22, 23, 24, 26], i = [1, 15], l = [1, 16], c = [1, 17], y = [1, 18], u = [1, 19], x = [1, 20], g = [1, 24], f = [4, 6, 9, 11, 17, 18, 20, 22, 23, 24, 26], d = {
    trace: function() {
    },
    yy: {},
    ...
    performAction: function(r, a, h, p, m, o, R) {
      var k = o.length - 1;
      switch (m) {
        case 1:
          return o[k - 1];
    ...
    parse: function(r) {
      var a = this, h = [0], p = [], m = [null], o = [], R = this.table, k = "", j = 0, Q = 0, yt = 2, tt = 1, dt = o.slice.call(arguments, 1), v = Object.create(this.lexer), A = { yy: {} };
      ...
      for (var w, I, $, X, F = {}, Y, M, et, q; ; ) {
        if (I = h[h.length - 1], this.defaultActions[I] ? $ = this.defaultActions[I] : ((w === null || typeof w > "u") && (w = ft()), $ = R[I] && R[I][w]), typeof $ > "u" || !$.length || !$[0]) {
          var G = "";
          q = [];
          for (Y in R[I])
            this.terminals_[Y] && Y > yt && q.push("'" + this.terminals_[Y] + "'");
          v.showPosition ? G = "Parse error on line " + (j + 1) + `:
` + v.showPosition() + `
Expecting ` + q.join(", ") + ", got '" + (this.terminals_[w] || w) + "'" : G = "Parse error on line " + (j + 1) + ": Unexpected " + (w == tt ? "end of input" : "'" + (this.terminals_[w] || w) + "'"), this.parseError(G, {
            text: v.match,
        ...
        switch ($[0]) {
          case 1:
            h.push(w), m.push(v.yytext), o.push(v.yylloc), h.push($[1]), w = null, Q = v.yyleng, k = v.yytext, j = v.yylineno, W = v.yylloc;
            break;
          case 2:
            if (M = this.productions_[$[1]][1], F.$ = m[m.length - M], F._$ = {
              ...
              return X;
            M && (h = h.slice(0, -1 * M * 2), m = m.slice(0, -1 * M), o = o.slice(0, -1 * M)), h.push(this.productions_[$[1]][0]), m.push(F.$), o.push(F._$), et = R[h[h.length - 2]][h[h.length - 1]], h.push(et);
            break;
          case 3:
            return !0;
        }
        ...
        for (var m = this._currentRules(), o = 0; o < m.length; o++)
          if (h = this._input.match(this.rules[m[o]]), h && (!a || h[0].length > a[0].length)) {
            if (a = h, p = o, this.options.backtrack_lexer) {
              if (r = this.test_match(h, m[o]), r !== !1)
                return r;
              if (this._backtrack) {
                a = !1;
                continue;
...
`, zt = Bt, K = function(t, e) {
  ...
  return e.score > 3 ? l(i) : e.score < 3 ? c(i) : y(i), n;
}, lt = function(t, e) {
...
const Ot = function(t, e, s) {
  ...
  nt++;
  const l = 300 + 5 * 30;
  i.append("line").attr("id", "task" + nt).attr("x1", n).attr("y1", e.y).attr("x2", n).attr("y2", l).attr("class", "task-line").attr("stroke-width", "1px").attr("stroke-dasharray", "4 2").attr("stroke", "#666"), jt(i, {
    cx: n,
    cy: 300 + (5 - e.score) * 30,
    score: e.score
  });
  ...
  function e(i, l, c, y, u, x, g, f, d) {
    ...
    for (let r = 0; r < _.length; r++) {
      const a = r * T - T * (_.length - 1) / 2, h = l.append("text").attr("x", c + u / 2).attr("y", y).attr("fill", d).style("text-anchor", "middle").style("font-size", T).style("font-family", b);
...
`);
  ...
  }), Gt(u), S.insert(0, 0, C, Object.keys(P).length * 50), Ut(u, x, 0);
  const T = S.getBounds();
  ...
  updateBounds: function(t, e, s, n) {
    ...
    function y(u) {
      return function(g) {
        ...
        l.updateVal(g, "starty", e - f * i.boxMargin, Math.min), l.updateVal(g, "stopy", n + f * i.boxMargin, Math.max), l.updateVal(S.data, "startx", t - f * i.boxMargin, Math.min), l.updateVal(S.data, "stopx", s + f * i.boxMargin, Math.max), u !== "activation" && (l.updateVal(g, "startx", t - f * i.boxMargin, Math.min), l.updateVal(g, "stopx", s + f * i.boxMargin, Math.max), l.updateVal(S.data, "starty", e - f * i.boxMargin, Math.min), l.updateVal(S.data, "stopy", n + f * i.boxMargin, Math.max));
      };
    }
    this.sequenceItems.forEach(y());
  },
  insert: function(t, e, s, n) {
    const i = Math.min(t, s), l = Math.max(t, s), c = Math.min(e, n), y = Math.max(e, n);
    this.updateVal(S.data, "startx", i, Math.min), this.updateVal(S.data, "starty", c, Math.min), this.updateVal(S.data, "stopx", l, Math.max), this.updateVal(S.data, "stopy", y, Math.max), this.updateBounds(i, c, l, y);
  },
  ...
  const n = V().journey;
  let i = "";
  const l = n.height * 2 + n.diagramMarginY, c = s + l;
  ...
  for (const [f, d] of e.entries()) {
    if (i !== d.section) {
      ...
      z.drawSection(t, r, n), i = d.section, y++;
    }
    const T = d.people.reduce((b, _) => (P[_] && (b[_] = P[_]), b), {});
    d.x = f * n.taskMargin + f * n.width + C, d.y = c, d.width = n.diagramMarginX, d.height = n.diagramMarginY, d.colour = x, d.fill = u, d.num = g, d.actors = T, z.drawTask(t, d, n), S.insert(d.x, d.y, d.x + d.width + n.taskMargin, 300 + 5 * 30);
  }
}, ct = {
  setConf: Xt,
  draw: Ht

import { c as A, s as Ae, g as Oe, a as Se, b as Ie, z as Ge, A as Pe, m as He, l as G, j as V, B as Ne, F as pe, h as De, x as Ve, G as ze, H as je } from "./index-0c19e849.js";
import "vue";
var me = function() {
  var r = function(B, l, u, b) {
    for (u = u || {}, b = B.length; b--; u[B[b]] = l)
      ;
    return u;
  }, a = [1, 4], o = [1, 7], h = [1, 5], c = [1, 9], n = [1, 6], f = [2, 6], p = [1, 16], v = [6, 8, 14, 20, 22, 24, 25, 27, 29, 32, 37, 40, 50, 55], y = [8, 14, 20, 22, 24, 25, 27, 29, 32, 37, 40], m = [8, 13, 14, 20, 22, 24, 25, 27, 29, 32, 37, 40], d = [1, 26], _ = [6, 8, 14, 50, 55], s = [8, 14, 55], C = [1, 53], w = [1, 52], N = [8, 14, 30, 33, 35, 38, 55], x = [1, 67], g = [1, 68], k = [1, 69], P = [8, 14, 33, 35, 42, 55], ae = {
    trace: function() {
    },
    yy: {},
...
const Ye = function(r, a, o) {
  He.parseDirective(this, r, a, o);
};
function Fe(r, a) {
  const o = /* @__PURE__ */ Object.create(null);
  return r.reduce((h, c) => {
    const n = a(c);
    return o[n] || (o[n] = !0, h.push(c)), h;
  }, []);
}
const Ke = function(r) {
  xe = r;
...
const Ue = function(r) {
  ...
  G.debug("Entering commit:", r, a, o, h), a = V.sanitizeText(a, A()), r = V.sanitizeText(r, A()), h = V.sanitizeText(h, A());
  const c = {
    id: a || K + "-" + fe(),
    message: r,
    seq: K++,
  ...
  if (r = V.sanitizeText(r, A()), T[r] === void 0)
    T[r] = O != null ? O.id : null, Q[r] = { name: r, order: a ? parseInt(a, 10) : null }, Ee(r), G.debug("in createBranch");
  else {
    let o = new Error(
  ...
  r = V.sanitizeText(r, A()), a = V.sanitizeText(a, A());
  const c = R[T[M]], n = R[T[r]];
  if (M === r) {
  ...
  if (G.debug("Entering cherryPick:", r, a, o), r = V.sanitizeText(r, A()), a = V.sanitizeText(a, A()), o = V.sanitizeText(o, A()), !r || R[r] === void 0) {
    let n = new Error(
...
function ve(r) {
  const a = r.reduce((c, n) => c.seq > n.seq ? c : n, r[0]);
  let o = "";
  ...
  for (let c in T)
    T[c] === a.id && h.push(c);
  if (G.debug(h.join(" ")), a.parents && a.parents.length == 2) {
    const c = R[a.parents[0]];
    ke(r, a, c), r.push(R[a.parents[1]]);
  } else {
...
const $e = function() {
  ...
  T = {}, T[r] = null, Q = {}, Q[r] = { name: r, order: a }, M = r, K = 0, Ne();
}, tt = function() {
  return Object.values(Q).map((a, o) => a.order !== null ? a : {
    ...a,
...
const ot = () => {
  ...
  for (const h of o) {
    ...
    c.setAttributeNS("http://www.w3.org/XML/1998/namespace", "xml:space", "preserve"), c.setAttribute("dy", "1em"), c.setAttribute("x", "0"), c.setAttribute("class", "row"), c.textContent = h.trim(), a.appendChild(c);
  }
  return a;
}, _e = (r, a, o) => {
  const h = pe().gitGraph, c = r.append("g").attr("class", "commit-bullets"), n = r.append("g").attr("class", "commit-labels");
  let f = 0;
  Object.keys(a).sort((y, m) => a[y].seq - a[m].seq).forEach((y) => {
    const m = a[y], d = I[m.branch].pos, _ = f + 10;
    if (o) {
    ...
    if (re[m.id] = { x: f + 10, y: d }, o) {
      if (m.type !== H.CHERRY_PICK && (m.customId && m.type === H.MERGE || m.type !== H.MERGE) && h.showCommitLabel) {
        const w = n.append("g"), N = w.insert("rect").attr("class", "commit-label-bkg"), x = w.append("text").attr("x", f).attr("y", d + 25).attr("class", "commit-label").text(m.id);
      ...
      if (m.tag) {
        const w = n.insert("polygon"), N = n.append("circle"), x = n.append("text").attr("y", d - 16).attr("class", "tag-label").text(m.tag);
        let g = x.node().getBBox();
...
}, ht = (r, a, o) => Object.keys(o).filter((n) => o[n].branch === a.branch && o[n].seq > r.seq && o[n].seq < a.seq).length > 0, ue = (r, a, o = 0) => {
  ...
  return ue(r, a - n / 5, o + 1);
}, mt = (r, a, o, h) => {
  const c = re[a.id], n = re[o.id], f = ht(a, o, h);
  let p = "", v = "", y = 0, m = 0, d = I[o.branch].index, _;
  if (f) {
    p = "A 10 10, 0, 0, 0,", v = "A 10 10, 0, 0, 1,", y = 10, m = 10, d = I[o.branch].index;
    const s = c.y < n.y ? ue(c.y, n.y) : ue(n.y, c.y);
    c.y < n.y ? _ = `M ${c.x} ${c.y} L ${c.x} ${s - y} ${p} ${c.x + m} ${s} L ${n.x - y} ${s} ${v} ${n.x} ${s + m} L ${n.x} ${n.y}` : _ = `M ${c.x} ${c.y} L ${c.x} ${s + y} ${v} ${c.x + m} ${s} L ${n.x - y} ${s} ${p} ${n.x} ${s - m} L ${n.x} ${n.y}`;
  } else
    c.y < n.y && (p = "A 20 20, 0, 0, 0,", y = 20, m = 20, d = I[o.branch].index, _ = `M ${c.x} ${c.y} L ${c.x} ${n.y - y} ${p} ${c.x + m} ${n.y} L ${n.x} ${n.y}`), c.y > n.y && (p = "A 20 20, 0, 0, 0,", y = 20, m = 20, d = I[a.branch].index, _ = `M ${c.x} ${c.y} L ${n.x - y} ${c.y} ${p} ${n.x} ${c.y - m} L ${n.x} ${n.y}`), c.y === n.y && (d = I[a.branch].index, _ = `M ${c.x} ${c.y} L ${c.x} ${n.y - y} ${p} ${c.x + m} ${n.y} L ${n.x} ${n.y}`);
  r.append("path").attr("d", _).attr("class", "arrow arrow" + d % F);
}, ut = (r, a) => {
  const o = r.append("g").attr("class", "commit-arrows");
  Object.keys(a).forEach((h) => {
    const c = a[h];
    c.parents && c.parents.length > 0 && c.parents.forEach((n) => {
      mt(o, a[n], c, a);
    });
  });
}, pt = (r, a) => {
  ...
  a.forEach((c, n) => {
    const f = n % F, p = I[c.name].pos, v = h.append("line");
  ...
  ot();
  const c = pe(), n = c.gitGraph;
  G.debug("in gitgraph renderer", r + `
`, "id:", a, o), J = h.db.getCommits();
  const f = h.db.getBranchesAsObjArray();
  let p = 0;
  f.forEach((y, m) => {
    I[y.name] = { pos: p, index: m }, p += 50 + (n.rotateCommitLabel ? 40 : 0);
  });
  const v = De(`[id="${a}"]`);
  _e(v, J, !1), n.showBranches && pt(v, f), ut(v, J), _e(v, J, !0), Ve.insertTitle(
    v,
    "gitTitleText",
    n.titleTopMargin,

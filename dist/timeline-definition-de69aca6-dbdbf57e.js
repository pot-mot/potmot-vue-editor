import { ar as bt, as as vt, A as kt, c as wt, l as I, h as U, t as St, at as Et, au as Tt } from "./index-81e19fd4.js";
import { i as It } from "./is_dark-999f2cab.js";
import { a as it } from "./arc-b198dff9.js";
import "vue";
...
var Y = function() {
  var n = function(b, r, a, h) {
    ...
    parse: function(r) {
      var a = this, h = [0], u = [], f = [null], c = [], T = this.table, m = "", M = 0, V = 0, j = 2, K = 1, H = c.slice.call(arguments, 1), p = Object.create(this.lexer), E = { yy: {} };
      ...
      for (var k, R, N, Q, W = {}, G, A, nt, q; ; ) {
        if (R = h[h.length - 1], this.defaultActions[R] ? N = this.defaultActions[R] : ((k === null || typeof k > "u") && (k = et()), N = T[R] && T[R][k]), typeof N > "u" || !N.length || !N[0]) {
          var X = "";
          q = [];
          for (G in T[R])
...
Expecting ` + q.join(", ") + ", got '" + (this.terminals_[k] || k) + "'" : X = "Parse error on line " + (M + 1) + ": Unexpected " + (k == K ? "end of input" : "'" + (this.terminals_[k] || k) + "'"), this.parseError(X, {
        ...
        switch (N[0]) {
          case 1:
            h.push(k), f.push(p.yytext), c.push(p.yylloc), h.push(N[1]), k = null, V = p.yyleng, m = p.yytext, M = p.yylineno, C = p.yylloc;
            break;
          case 2:
            if (A = this.productions_[N[1]][1], W.$ = f[f.length - A], W._$ = {
              ...
              return Q;
            A && (h = h.slice(0, -1 * A * 2), f = f.slice(0, -1 * A), c = c.slice(0, -1 * A)), h.push(this.productions_[N[1]][0]), f.push(W.$), c.push(W._$), nt = T[h[h.length - 2]][h[h.length - 1]], h.push(nt);
            break;
          case 3:
            return !0;
        }
...
` + this.showPosition(), {
  ...
  return $.prototype = _, _.Parser = $, new $();
}();
Y.parser = Y;
const Nt = Y;
let B = "", ct = 0;
...
const Vt = function(n, t, e) {
  ...
  function t(i, o, l, g, d, y, x, w, v) {
    const { taskFontSize: _, taskFontFamily: S } = w, $ = i.split(/<br\s*\/?>/gi);
    for (let b = 0; b < $.length; b++) {
      const r = b * _ - _ * ($.length - 1) / 2, a = o.append("text").attr("x", l + d / 2).attr("y", g).attr("fill", v).style("text-anchor", "middle").style("font-size", _).style("font-family", S);
      a.append("tspan").attr("x", l + d / 2).attr("dy", r).text($[b]), a.attr("y", g + y / 2).attr("dominant-baseline", "central").attr("alignment-baseline", "central"), s(a, x);
    }
  }
...
`), I.debug("timeline", s.db);
  ...
  const _ = (x === "sandbox" ? (
    ...
    U(w.nodes()[0].contentDocument.body)
  ) : U("body")).select("#" + t);
  _.append("g");
  const S = s.db.getTasks(), $ = s.db.getCommonDb().getDiagramTitle();
  ...
  I.debug("maxSectionHeight before draw", r), I.debug("maxTaskHeight before draw", a), b && b.length > 0 ? b.forEach((H) => {
    const p = S.filter((z) => z.section === H), E = {
      number: T,
      descr: H,
      section: T,
  ...
  I.debug("bounds", j), $ && _.append("text").text($).attr("x", j.width / 2 - y).attr("font-size", "4ex").attr("font-weight", "bold").attr("y", 20), h = m ? r + a + 150 : a + 100, _.append("g").attr("class", "lineWrapper").append("line").attr("x1", y).attr("y1", h).attr("x2", j.width + 3 * y).attr("y2", h).attr("stroke-width", 4).attr("stroke", "black").attr("marker-end", "url(#arrowhead)"), St(
    void 0,
    _,

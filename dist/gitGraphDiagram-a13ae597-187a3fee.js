import { c as I, s as Oe, g as Ie, a as Ge, b as Pe, B as He, D as Ne, m as Be, l as H, j as V, E as De, H as fe, h as Ve, z as ze, I as je, J as qe } from "./index-9a635572.js";
import "vue";
var pe = function() {
  var r = function(q, m, b, k) {
    for (b = b || {}, k = q.length; k--; b[q[k]] = m)
      ;
    return b;
  }, a = [1, 4], o = [1, 7], p = [1, 5], n = [1, 9], c = [1, 6], u = [2, 6], h = [1, 16], v = [6, 8, 14, 20, 22, 24, 25, 27, 29, 32, 37, 40, 50, 55], x = [8, 14, 20, 22, 24, 25, 27, 29, 32, 37, 40], l = [8, 13, 14, 20, 22, 24, 25, 27, 29, 32, 37, 40], g = [1, 26], f = [6, 8, 14, 50, 55], s = [8, 14, 55], _ = [1, 53], w = [1, 52], O = [8, 14, 30, 33, 35, 38, 55], E = [1, 67], d = [1, 68], y = [1, 69], N = [8, 14, 33, 35, 42, 55], ce = {
    trace: function() {
    },
    yy: {},
    ...
    performAction: function(m, b, k, i, T, e, W) {
      var t = e.length - 1;
      switch (T) {
        case 3:
          return e[t];
    ...
    parse: function(m) {
      var b = this, k = [0], i = [], T = [null], e = [], W = this.table, t = "", re = 0, ge = 0, Me = 2, de = 1, Ce = e.slice.call(arguments, 1), M = Object.create(this.lexer), Y = { yy: {} };
...
Expecting ` + se.join(", ") + ", got '" + (this.terminals_[P] || P) + "'" : ue = "Parse error on line " + (re + 1) + ": Unexpected " + (P == de ? "end of input" : "'" + (this.terminals_[P] || P) + "'"), this.parseError(ue, {
        ...
        switch (D[0]) {
          case 1:
            k.push(P), T.push(M.yytext), e.push(M.yylloc), k.push(D[1]), P = null, ge = M.yyleng, t = M.yytext, re = M.yylineno, he = M.yylloc;
            break;
          case 2:
            if (z = this.productions_[D[1]][1], J.$ = T[T.length - z], J._$ = {
              ...
              return me;
            z && (k = k.slice(0, -1 * z * 2), T = T.slice(0, -1 * z), e = e.slice(0, -1 * z)), k.push(this.productions_[D[1]][0]), T.push(J.$), e.push(J._$), ke = W[k[k.length - 2]][k[k.length - 1]], k.push(ke);
            break;
          case 3:
            return !0;
        }
...
` + b + "^";
      ...
      test_match: function(m, b) {
        ...
        }, this.options.ranges && (T.yylloc.range = this.yylloc.range.slice(0))), i = m[0].match(/(?:\r\n?|\n).*/g), i && (this.yylineno += i.length), this.yylloc = {
      ...
      next: function() {
        ...
        for (var T = this._currentRules(), e = 0; e < T.length; e++)
          if (k = this._input.match(this.rules[T[e]]), k && (!b || k[0].length > b[0].length)) {
            if (b = k, i = e, this.options.backtrack_lexer) {
              if (m = this.test_match(k, T[e]), m !== !1)
                return m;
              if (this._backtrack) {
                b = !1;
                continue;
        ...
        return b ? (m = this.test_match(b, T[i]), m !== !1 ? m : !1) : this._input === "" ? this.EOF : this.parseError("Lexical error on line " + (this.yylineno + 1) + `. Unrecognized text.
` + this.showPosition(), {
          text: "",
          token: null,
          line: this.yylineno
      ...
      performAction: function(b, k, i, T) {
        switch (i) {
          case 0:
...
const We = function(r) {
  ...
  if (r = V.sanitizeText(r, I()), L[r] === void 0)
    L[r] = G != null ? G.id : null, Z[r] = { name: r, order: a ? parseInt(a, 10) : null }, ve(r), H.debug("in createBranch");
  else {
    let o = new Error(
...
const ht = () => {
  ...
  S === "TB" && (u = 30), Object.keys(a).sort((x, l) => a[x].seq - a[l].seq).forEach((x) => {
    ...
    if (o) {
      ...
      if (_ === B.HIGHLIGHT) {
        const w = n.append("rect");
        w.attr("x", f - 10), w.attr("y", g - 10), w.attr("height", 20), w.attr("width", 20), w.attr(
          "class",
        ...
        const w = n.append("circle");
        if (w.attr("cx", f), w.attr("cy", g), w.attr("r", l.type === B.MERGE ? 9 : 10), w.attr(
          "class",
    ...
    if (S === "TB" ? ee[l.id] = { x: f, y: u + 10 } : ee[l.id] = { x: u + 10, y: g }, o) {
      if (l.type !== B.CHERRY_PICK && (l.customId && l.type === B.MERGE || l.type !== B.MERGE) && p.showCommitLabel) {
        const w = c.append("g"), O = w.insert("rect").attr("class", "commit-label-bkg"), E = w.append("text").attr("x", u).attr("y", g + 25).attr("class", "commit-label").text(l.id);
      ...
      if (l.tag) {
        const w = c.insert("polygon"), O = c.append("circle"), E = c.append("text").attr("y", g - 16).attr("class", "tag-label").text(l.tag);
        let d = E.node().getBBox();
        ...
        w.attr("class", "tag-label-bkg").attr(
          ...
          ${u + 10 + d.width / 2 + 4},${N + y + 2}
          ${u + 10 - d.width / 2 - 4},${N + y + 2}`
        ), O.attr("cx", u - d.width / 2 + 4 / 2).attr("cy", N).attr("r", 1.5).attr("class", "tag-hole"), S === "TB" && (w.attr("class", "tag-label-bkg").attr(
          "points",
          `
            ${f},${u + 2}
            ${f},${u - 2}
  ...
  let h = "", v = "", x = 0, l = 0, g = C[o.branch].index, f;
  if (u) {
    h = "A 10 10, 0, 0, 0,", v = "A 10 10, 0, 0, 1,", x = 10, l = 10, g = C[o.branch].index;
    const s = n.y < c.y ? Q(n.y, c.y) : Q(c.y, n.y), _ = n.x < c.x ? Q(n.x, c.x) : Q(c.x, n.x);
    S === "TB" ? n.x < c.x ? f = `M ${n.x} ${n.y} L ${_ - x} ${n.y} ${v} ${_} ${n.y + l} L ${_} ${c.y - x} ${h} ${_ + l} ${c.y} L ${c.x} ${c.y}` : f = `M ${n.x} ${n.y} L ${_ + x} ${n.y} ${h} ${_} ${n.y + l} L ${_} ${c.y - x} ${v} ${_ - l} ${c.y} L ${c.x} ${c.y}` : n.y < c.y ? f = `M ${n.x} ${n.y} L ${n.x} ${s - x} ${h} ${n.x + l} ${s} L ${c.x - x} ${s} ${v} ${c.x} ${s + l} L ${c.x} ${c.y}` : f = `M ${n.x} ${n.y} L ${n.x} ${s + x} ${v} ${n.x + l} ${s} L ${c.x - x} ${s} ${h} ${c.x} ${s - l} L ${c.x} ${c.y}`;
  } else
  ...
  a.forEach((n, c) => {
    const u = c % F, h = C[n.name].pos, v = p.append("line");
    v.attr("x1", 0), v.attr("y1", h), v.attr("x2", te), v.attr("y2", h), v.attr("class", "branch branch" + u), S === "TB" && (v.attr("y1", 30), v.attr("x1", h), v.attr("y2", te), v.attr("x2", h)), ne.push(h);
    let x = n.name;
...
`, "id:", a, o), X = p.db.getCommits();
  ...
  u.forEach((x, l) => {
    ...
    let w = g.getBBox();
    C[x.name] = { pos: v, index: l }, v += 50 + (c.rotateCommitLabel ? 40 : 0) + (S === "TB" ? w.width / 2 : 0), _.remove(), s.remove(), f.remove();
  }), _e(h, X, !1), c.showBranches && ft(h, u), pt(h, X), _e(h, X, !0), ze.insertTitle(
    h,
    "gitTitleText",
    c.titleTopMargin,

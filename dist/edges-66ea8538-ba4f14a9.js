import { p as H, c as u, d as Z, aw as j, h as L, l as x, x as D, D as F } from "./index-14ad5708.js";
import { c as P } from "./createText-a49d2d2a-15fbd547.js";
import { p as Q } from "./svgDraw-95adee0a-ef37635e.js";
import { l as et } from "./line-20e46e13.js";
const st = (r, t, e, n) => {
  t.forEach((a) => {
    yt[a](r, e, n);
  });
...
function xt(r) {
  const t = L(document.createElementNS("http://www.w3.org/2000/svg", "foreignObject")), e = t.append("xhtml:div"), n = r.label, a = r.isNode ? "nodeLabel" : "edgeLabel";
  return e.html(
...
const gt = (r, t, e, n) => {
  let a = r || "";
  if (typeof a == "object" && (a = a[0]), H(u().flowchart.htmlLabels)) {
...
}, T = gt, S = async (r, t, e, n) => {
  let a;
  const s = t.useHtmlLabels || H(u().flowchart.htmlLabels);
  ...
  const o = l.node();
  let h;
  t.labelType === "markdown" ? h = P(l, Z(j(c), u()), {
    useHtmlLabels: s,
  ...
  if (H(u().flowchart.htmlLabels)) {
    const p = h.children[0], d = L(h), y = p.getElementsByTagName("img");
    if (y) {
      const g = c.replace(/<img[^>]*>/g, "").trim() === "";
      await Promise.all(
        [...y].map(
          (w) => new Promise(
            (R) => w.addEventListener("load", function() {
              if (w.style.display = "flex", w.style.flexDirection = "column", g) {
                const I = u().fontSize ? u().fontSize : window.getComputedStyle(document.body).fontSize, E = 5;
                w.style.width = parseInt(I, 10) * E + "px";
              } else
                w.style.width = "100%";
              R(w);
            })
...
function mt(r, t, e, n) {
  var a, s, i, l, c, o, h, f, b, p, d, y, g, w, R;
  if (a = t.y - r.y, i = r.x - t.x, c = t.x * r.y - r.x * t.y, b = a * e.x + i * e.y + c, p = a * n.x + i * n.y + c, !(b !== 0 && p !== 0 && V(b, p)) && (s = n.y - e.y, l = e.x - n.x, o = n.x * e.y - e.x * n.y, h = s * r.x + l * r.y + o, f = s * t.x + l * t.y + o, !(h !== 0 && f !== 0 && V(h, f)) && (d = a * l - s * i, d !== 0)))
    return y = Math.abs(d / 2), g = i * o - l * c, w = g < 0 ? (g - y) / d : (g + y) / d, g = s * c - a * o, R = g < 0 ? (g - y) / d : (g + y) / d, { x: w, y: R };
}
function V(r, t) {
  return r * t > 0;
}
function kt(r, t, e) {
  ...
  return s.length ? (s.length > 1 && s.sort(function(d, y) {
    var g = d.x - e.x, w = d.y - e.y, R = Math.sqrt(g * g + w * w), I = y.x - e.x, E = y.y - e.y, X = Math.sqrt(I * I + E * E);
...
const vt = (r, t) => {
  ...
  return N(e, a, s, i).attr("style", t.style), t.width = a + s, t.height = s, t.intersect = function(c) {
...
const Xt = (r, t) => {
  ...
  ), a.attr("class", "outer title-state").attr("x", -h.width / 2 - d).attr("y", -h.height / 2 - d).attr("width", h.width + t.padding).attr("height", h.height + t.padding), s.attr("class", "divider").attr("x1", -h.width / 2 - d).attr("x2", h.width / 2 + d).attr("y1", -h.height / 2 - d + b.height + d).attr("y2", -h.height / 2 - d + b.height + d), k(t, a), t.intersect = function(y) {
    return m.rect(t, y);
  }, n;
}, Yt = async (r, t) => {
  ...
  let p = 0;
  const d = t.classData.annotations && t.classData.annotations[0], y = t.classData.annotations[0] ? "«" + t.classData.annotations[0] + "»" : "", g = b.node().appendChild(T(y, t.labelStyle, !0, !0));
  let w = g.getBBox();
  if (H(u().flowchart.htmlLabels)) {
    const B = g.children[0], M = L(g);
    w = B.getBoundingClientRect(), M.attr("width", w.width), M.attr("height", w.height);
  }
  t.classData.annotations[0] && (f += w.height + n, h += w.width);
  let R = t.classData.label;
  t.classData.type !== void 0 && t.classData.type !== "" && (u().flowchart.htmlLabels ? R += "&lt;" + t.classData.type + "&gt;" : R += "<" + t.classData.type + ">");
  const I = b.node().appendChild(T(R, t.labelStyle, !0, !0));
  L(I).attr("class", "classTitle");
  let E = I.getBBox();
  if (H(u().flowchart.htmlLabels)) {
    const B = I.children[0], M = L(I);
  ...
  t.classData.members.forEach((B) => {
    const M = Q(B);
    let _ = M.displayText;
    u().flowchart.htmlLabels && (_ = _.replace(/</g, "&lt;").replace(/>/g, "&gt;"));
    const $ = b.node().appendChild(
      T(
        _,
    ...
    let C = $.getBBox();
    if (H(u().flowchart.htmlLabels)) {
      const O = $.children[0], Y = L($);
      C = O.getBoundingClientRect(), Y.attr("width", C.width), Y.attr("height", C.height);
    }
    C.width > h && (h = C.width), f += C.height + n, X.push($);
  }), f += a;
  const z = [];
  ...
  ), p += E.height + n, c.attr("class", "divider").attr("x1", -h / 2 - e).attr("x2", h / 2 + e).attr("y1", -f / 2 - e + a + p).attr("y2", -f / 2 - e + a + p), p += a, X.forEach((B) => {
    L(B).attr(
      "transform",
      "translate( " + -h / 2 + ", " + (-1 * f / 2 + p + a / 2) + ")"
    ), p += E.height + n;
  }), p += a, o.attr("class", "divider").attr("x1", -h / 2 - e).attr("x2", h / 2 + e).attr("y1", -f / 2 - e + a + p).attr("y2", -f / 2 - e + a + p), p += a, z.forEach((B) => {
    L(B).attr(
      "transform",
...
const Gt = async (r, t, e) => {
  ...
  if (t.link) {
    let s;
    u().securityLevel === "sandbox" ? s = "_top" : t.linkTarget && (s = t.linkTarget || "_blank"), n = r.insert("svg:a").attr("xlink:href", t.link).attr("target", s), a = await K[t.shape](n, t, e);
  } else
...
const ar = () => {
  ...
  const e = H(u().flowchart.htmlLabels), n = t.labelType === "markdown" ? P(r, t.label, {
    style: t.labelStyle,
    useHtmlLabels: e,
    addSvgBackground: !0
  ...
  if (t.startLabelLeft) {
    ...
    const f = c.getBBox();
    h.attr("transform", "translate(" + -f.width / 2 + ", " + -f.height / 2 + ")"), v[t.id] || (v[t.id] = {}), v[t.id].startLeft = o, U(l, t.startLabelLeft);
  }
  if (t.startLabelRight) {
    ...
    const f = c.getBBox();
    h.attr("transform", "translate(" + -f.width / 2 + ", " + -f.height / 2 + ")"), v[t.id] || (v[t.id] = {}), v[t.id].startRight = o, U(l, t.startLabelRight);
  }
  if (t.endLabelLeft) {
    const c = T(t.endLabelLeft, t.labelStyle), o = r.insert("g").attr("class", "edgeTerminals"), h = o.insert("g").attr("class", "inner");
    l = h.node().appendChild(c);
    const f = c.getBBox();
    h.attr("transform", "translate(" + -f.width / 2 + ", " + -f.height / 2 + ")"), o.node().appendChild(c), v[t.id] || (v[t.id] = {}), v[t.id].endLeft = o, U(l, t.endLabelLeft);
  }
  if (t.endLabelRight) {
    const c = T(t.endLabelRight, t.labelStyle), o = r.insert("g").attr("class", "edgeTerminals"), h = o.insert("g").attr("class", "inner");
    l = h.node().appendChild(c);
    const f = c.getBBox();
    h.attr("transform", "translate(" + -f.width / 2 + ", " + -f.height / 2 + ")"), o.node().appendChild(c), v[t.id] || (v[t.id] = {}), v[t.id].endRight = o, U(l, t.endLabelRight);
  }
  return n;
};
function U(r, t) {
...
const sr = (r, t) => {
  x.info("Moving label abc78 ", r.id, r.label, A[r.id]);
  ...
  if (Math.abs(a - t.y) * i > Math.abs(n - t.x) * c) {
    ...
    let f = o * l / h, b = e.x < t.x ? e.x + h - l : e.x - h + l, p = e.y < t.y ? e.y + f : e.y - f;
  ...
  let y = "";
  switch ((u().flowchart.arrowMarkerAbsolute || u().state.arrowMarkerAbsolute) && (y = window.location.protocol + "//" + window.location.host + window.location.pathname + window.location.search, y = y.replace(/\(/g, "\\("), y = y.replace(/\)/g, "\\)")), x.info("arrowTypeStart", e.arrowTypeStart), x.info("arrowTypeEnd", e.arrowTypeEnd), e.arrowTypeStart) {
    case "arrow_cross":
      d.attr("marker-start", "url(" + y + "#" + a + "-crossStart)");
      break;
    case "arrow_point":
      d.attr("marker-start", "url(" + y + "#" + a + "-pointStart)");
      break;
    case "arrow_barb":
      d.attr("marker-start", "url(" + y + "#" + a + "-barbStart)");
      break;
    case "arrow_circle":
      d.attr("marker-start", "url(" + y + "#" + a + "-circleStart)");
      break;
    case "aggregation":
      d.attr("marker-start", "url(" + y + "#" + a + "-aggregationStart)");
      break;
    case "extension":
      d.attr("marker-start", "url(" + y + "#" + a + "-extensionStart)");
      break;
    case "composition":
      d.attr("marker-start", "url(" + y + "#" + a + "-compositionStart)");
      break;
    case "dependency":
      d.attr("marker-start", "url(" + y + "#" + a + "-dependencyStart)");
      break;
    case "lollipop":
      d.attr("marker-start", "url(" + y + "#" + a + "-lollipopStart)");
      break;
  }
  switch (e.arrowTypeEnd) {
    case "arrow_cross":
      d.attr("marker-end", "url(" + y + "#" + a + "-crossEnd)");
      break;
    case "arrow_point":
      d.attr("marker-end", "url(" + y + "#" + a + "-pointEnd)");
      break;
    case "arrow_barb":
      d.attr("marker-end", "url(" + y + "#" + a + "-barbEnd)");
      break;
    case "arrow_circle":
      d.attr("marker-end", "url(" + y + "#" + a + "-circleEnd)");
      break;
    case "aggregation":
      d.attr("marker-end", "url(" + y + "#" + a + "-aggregationEnd)");
      break;
    case "extension":
      d.attr("marker-end", "url(" + y + "#" + a + "-extensionEnd)");
      break;
    case "composition":
      d.attr("marker-end", "url(" + y + "#" + a + "-compositionEnd)");
      break;
    case "dependency":
      d.attr("marker-end", "url(" + y + "#" + a + "-dependencyEnd)");
      break;
    case "lollipop":
      d.attr("marker-end", "url(" + y + "#" + a + "-lollipopEnd)");
      break;
  }
  let g = {};

import { Q as Ca, au as mp, d as bp, c as nn, l as qr, h as ei, t as Ep, az as wp, aA as xp, aB as Tp } from "./index-7301cf54.js";
import { c as Cp } from "./createText-285e50b4-a738eb1a.js";
import "vue";
var Ki = {}, Dp = {
  get exports() {
    return Ki;
...
In order to be iterable, non-array objects must have a [Symbol.iterator]() method.`);
    ...
    function Vl(t, e, r) {
      ...
      function d(A) {
        var x = a, R = n;
        return a = n = void 0, u = A, s = t.apply(R, x), s;
      }
      function v(A) {
        return u = A, o = setTimeout(g, e), f ? d(A) : s;
      }
      function p(A) {
        var x = A - l, R = A - u, C = e - x;
        return h ? $l(C, i - R) : C;
      }
      function y(A) {
        var x = A - l, R = A - u;
        return l === void 0 || x >= e || x < 0 || h && R >= i;
      }
      function g() {
        var A = ai();
        if (y(A))
    ...
    var ln = Vl, ni = D ? D.performance : null, us = ni && ni.now ? function() {
      ...
      return e = {
        ...
        for (var A = function() {
          ...
          var re = c[B], W = h[B], Z = W != null ? W.source() : null, ie = W != null ? W.target() : null, ue = W == null ? void 0 : X.same(Z) ? ie[0] : Z[0], pe = void 0;
          if (pe = n(X, W, ue, v++, re), pe === !0)
            return p = X, "break";
          if (pe === !1)
            return "break";
          for (var se = X.connectedEdges().filter(function(ye) {
            ...
            var we = se[de], Te = we.connectedNodes().filter(function(ye) {
              return !ye.same(X) && g.has(ye);
            }), Ee = Te.id();
            Te.length !== 0 && !d[Ee] && (Te = Te[0], u.push(Te), e.bfs && (d[Ee] = !0, f.push(Te)), h[Ee] = we, c[Ee] = c[B] + 1);
          }
        }; u.length !== 0; ) {
          var x = A();
        ...
        for (var R = o.collection(), C = 0; C < f.length; C++) {
          var F = f[C], z = h[F.id()];
          z != null && R.push(z), R.push(F);
        }
        return {
          path: o.collection(R),
    ...
    var tf = il(function(t, e) {
        ...
        n = Math.floor, h = Math.min, a = function(g, m) {
          ...
          for (m == null && (m = a), A = function() {
            ...
            for (var C = 0, F = n(g.length / 2); 0 <= F ? C < F : C > F; 0 <= F ? C++ : C--)
              R.push(C);
            return R;
          }.apply(this).reverse(), x = [], E = 0, M = A.length; E < M; E++)
          ...
          if (b == null && (b = a), M = g.slice(0, m), !M.length)
            return M;
          for (i(M, b), R = g.slice(m), A = 0, x = R.length; A < x; A++)
            E = R[A], l(M, E, b);
          return M.sort(b).reverse();
        }, d = function(g, m, b) {
          var E, M, A, x, R, C, F, z, I;
          if (b == null && (b = a), m * 10 <= g.length) {
            if (A = g.slice(0, m).sort(b), !A.length)
              return A;
            for (M = A[A.length - 1], F = g.slice(m), x = 0, C = F.length; x < C; x++)
              E = F[x], b(E, M) < 0 && (f(A, E, 0, null, b), A.pop(), M = A[A.length - 1]);
            return A;
          }
          for (i(g, b), I = [], R = 0, z = h(m, g.length); 0 <= z ? R < z : R > z; 0 <= z ? ++R : --R)
            I.push(s(g, b));
          return I;
        }, p = function(g, m, b, E) {
          var M, A, x;
          for (E == null && (E = a), M = g[b]; b > m; ) {
            if (x = b - 1 >> 1, A = g[x], E(M, A) < 0) {
              g[b] = A, b = x;
              continue;
            }
            break;
          ...
          var E, M, A, x, R;
          for (b == null && (b = a), M = g.length, R = m, A = g[m], E = 2 * m + 1; E < M; )
            x = E + 1, x < M && !(b(g[E], g[x]) < 0) && (E = x), g[m] = g[E], m = E, E = 2 * m + 1;
          return g[m] = A, p(g, R, m, b);
        }, r = function() {
      ...
      dijkstra: function(e) {
        ...
        var a = rf(e), n = a.root, i = a.weight, s = a.directed, o = this, l = i, u = j(n) ? this.filter(n)[0] : n[0], f = {}, h = {}, c = {}, d = this.byGroup(), v = d.nodes, p = d.edges;
        ...
        for (var M = function(W, Z) {
          for (var ie = (s ? W.edgesTo(Z) : W.edgesWith(Z)).intersect(p), ue = 1 / 0, pe, se = 0; se < ie.length; se++) {
            var de = ie[se], we = l(de);
            (we < ue || !pe) && (ue = we, pe = de);
          }
          return {
            edge: pe,
            dist: ue
          ...
          var A = m.pop(), x = y(A), R = A.id();
          if (c[R] = x, x !== 1 / 0)
            for (var C = A.neighborhood().intersect(v), F = 0; F < C.length; F++) {
              var z = C[F], I = z.id(), X = M(A, z), B = x + X.dist;
              B < y(z) && (g(z, B), h[I] = {
                node: A,
                edge: X.edge
              });
        ...
        return {
          distanceTo: function(W) {
            var Z = j(W) ? v.filter(W)[0] : W[0];
            return c[Z.id()];
          },
          pathTo: function(W) {
            var Z = j(W) ? v.filter(W)[0] : W[0], ie = [], ue = Z, pe = ue.id();
            if (Z.length > 0)
              for (ie.unshift(Z); h[pe]; ) {
                var se = h[pe];
                ie.unshift(se.edge), ie.unshift(se.node), ue = se.node, pe = ue.id();
              }
            return o.spawn(ie);
          }
        };
      ...
      aStar: function(e) {
        ...
        var u = n.id(), f = i.id(), h = {}, c = {}, d = {}, v = new Ra(function(pe, se) {
          return c[pe.id()] - c[se.id()];
        }), p = new Jr(), y = {}, g = {}, m = function(se, de) {
          v.push(se), p.add(de);
        }, b, E, M = function() {
        ...
        for (var x = 0; v.size() > 0; ) {
          if (M(), x++, E === f) {
            for (var R = [], C = i, F = f, z = g[F]; R.unshift(C), z != null && R.unshift(z), C = y[F], C != null; )
              F = C.id(), z = g[F];
            return {
              found: !0,
              distance: h[E],
          ...
          d[E] = !0;
          for (var I = b._private.edges, X = 0; X < I.length; X++) {
            var B = I[X];
            if (this.hasElementWithId(B.id()) && !(o && B.data("source") !== E)) {
              var re = B.source(), W = B.target(), Z = re.id() !== E ? re : W, ie = Z.id();
      ...
      floydWarshall: function(e) {
        ...
        for (var m = new Array(h), b = new Array(h), E = 0; E < u.length; E++) {
          var M = u[E], A = M.source()[0], x = M.target()[0];
          if (A !== x) {
            var R = c(A), C = c(x), F = R * f + C, z = s(M);
            if (v[F] > z && (v[F] = z, m[F] = C, b[F] = M), !i) {
              var I = C * f + R;
              !i && v[I] > z && (v[I] = z, m[I] = R, b[I] = M);
            }
          }
        }
        for (var X = 0; X < f; X++)
          for (var B = 0; B < f; B++)
            for (var re = B * f + X, W = 0; W < f; W++) {
              var Z = B * f + W, ie = X * f + W;
        ...
        var ue = function(we) {
          ...
          distance: function(we, Te) {
            var Ee = pe(we), ye = pe(Te);
            return v[Ee * f + ye];
          },
          path: function(we, Te) {
            var Ee = pe(we), ye = pe(Te), ae = d(Ee);
            if (Ee === ye)
            ...
            for (xe.merge(ae); Ee !== ye; )
              Ce = Ee, Ee = m[Ee * f + ye], Oe = b[Ce * f + Ee], xe.merge(Oe), xe.merge(d(Ee));
            return xe;
          }
        };
        return se;
      ...
      bellmanFord: function(e) {
        var r = this, a = ff(e), n = a.weight, i = a.directed, s = a.root, o = n, l = this, u = this.cy(), f = this.byGroup(), h = f.edges, c = f.nodes, d = c.length, v = new ur(), p = !1, y = [];
        s = u.collection(s)[0], h.unmergeBy(function(Fe) {
          return Fe.isLoop();
        });
        for (var g = h.length, m = function(ke) {
          var Ge = v.get(ke.id());
          return Ge || (Ge = {}, v.set(ke.id(), Ge)), Ge;
        }, b = function(ke) {
          ...
          var x = c[A], R = m(x);
          x.same(s) ? R.dist = 0 : R.dist = 1 / 0, R.pred = null, R.edge = null;
        }
        for (var C = !1, F = function(ke, Ge, et, We, Ye, Se) {
          var N = We.dist + Se;
          N < Ye.dist && !et.same(We.edge) && (Ye.dist = N, Ye.pred = ke, Ye.edge = et, C = !0);
        }, z = 1; z < d; z++) {
          C = !1;
          for (var I = 0; I < g; I++) {
            var X = h[I], B = X.source(), re = X.target(), W = o(X), Z = m(B), ie = m(re);
            F(B, re, X, Z, ie, W), i || F(re, B, X, ie, Z, W);
          }
          if (!C)
            break;
        }
        if (C)
          for (var ue = [], pe = 0; pe < g; pe++) {
            var se = h[pe], de = se.source(), we = se.target(), Te = o(se), Ee = m(de).dist, ye = m(we).dist;
            if (Ee + Te < ye || !i && ye + Te < Ee)
              if (p || (ft("Graph contains a negative weight cycle for Bellman-Ford"), p = !0), e.findNegativeWeightCycles !== !1) {
                ...
                for (var xe = ae.length, Ce = 0; Ce < xe; Ce++) {
                  ...
                  ue.indexOf(Ve) === -1 && (y.push(l.spawn(Ie)), ue.push(Ve));
                }
              } else
                break;
          }
      ...
      kargerStein: function() {
        ...
        for (var c = 1 / 0, d = [], v = new Array(i), p = new Array(i), y = new Array(i), g = function(ue, pe) {
          for (var se = 0; se < i; se++)
            pe[se] = ue[se];
        }, m = 0; m <= o; m++) {
          for (var b = 0; b < i; b++)
            p[b] = b;
          var E = ui(p, u.slice(), i, l), M = E.slice();
          g(p, y);
          var A = ui(p, E, l, 2), x = ui(y, M, l, 2);
          A.length <= x.length && A.length < c ? (c = A.length, d = A, g(p, v)) : x.length <= A.length && x.length < c && (c = x.length, d = x, g(y, v));
        }
        for (var R = this.spawn(d.map(function(ie) {
          return n[ie[0]];
        })), C = this.spawn(), F = this.spawn(), z = v[0], I = 0; I < v.length; I++) {
          var X = v[I], B = a[I];
        ...
        var re = function(ue) {
          ...
          return ue.forEach(function(se) {
            pe.merge(se), se.connectedEdges().forEach(function(de) {
              e.contains(de) && !R.contains(de) && pe.merge(de);
            });
          }), pe;
        }, W = [re(C), re(F)], Z = {
          cut: R,
      ...
      var l = Fa(i, s), u = i / 2, f = s / 2, h;
      {
        ...
        if (h = Tr(e, r, a, n, c, d, v, p, !1), h.length > 0)
          return h;
      }
      {
        var y = a + u + o, g = n - f + l - o, m = y, b = n + f - l + o;
        if (h = Tr(e, r, a, n, y, g, m, b, !1), h.length > 0)
          return h;
      }
      {
        var E = a - u + l - o, M = n + f + o, A = a + u - l + o, x = M;
        if (h = Tr(e, r, a, n, E, M, A, x, !1), h.length > 0)
          return h;
      }
      {
        var R = a - u - o, C = n - f + l - o, F = R, z = n + f - l + o;
        if (h = Tr(e, r, a, n, R, C, F, z, !1), h.length > 0)
          return h;
      }
      var I;
      {
        var X = a - u + l, B = n - f + l;
        if (I = Pa(e, r, a, n, X, B, l + o), I.length > 0 && I[0] <= X && I[1] <= B)
          return [I[0], I[1]];
      }
      {
        var re = a + u - l, W = n - f + l;
        if (I = Pa(e, r, a, n, re, W, l + o), I.length > 0 && I[0] >= re && I[1] <= W)
          return [I[0], I[1]];
      }
      {
        var Z = a + u - l, ie = n + f - l;
        if (I = Pa(e, r, a, n, Z, ie, l + o), I.length > 0 && I[0] >= Z && I[1] >= ie)
          return [I[0], I[1]];
      }
      {
        var ue = a - u + l, pe = n + f - l;
        if (I = Pa(e, r, a, n, ue, pe, l + o), I.length > 0 && I[0] <= ue && I[1] >= pe)
          return [I[0], I[1]];
      }
      return [];
      ...
      p.push(1), p.push(0);
      for (var g = -1, m, b, E, M = 0; M < p.length; M++)
        m = Math.pow(1 - p[M], 2) * a + 2 * (1 - p[M]) * p[M] * i + p[M] * p[M] * o, b = Math.pow(1 - p[M], 2) * n + 2 * (1 - p[M]) * p[M] * s + p[M] * p[M] * l, E = Math.pow(m - e, 2) + Math.pow(b - r, 2), g >= 0 ? E < g && (g = E) : g = E;
      return g;
      ...
      for (var l = new Array(a.length), u = s / 2, f = o / 2, h = di(s, o), c = h * h, d = 0; d < a.length / 4; d++) {
        var v = void 0, p = void 0;
        d === 0 ? v = a.length - 2 : v = d * 4 - 2, p = d * 4 + 2;
        var y = n + u * a[d * 4], g = i + f * a[d * 4 + 1], m = -a[v] * a[p] - a[v + 1] * a[p + 1], b = h / Math.tan(Math.acos(m) / 2), E = y - b * a[v], M = g - b * a[v + 1], A = y + b * a[p], x = g + b * a[p + 1];
        l[d * 4] = E, l[d * 4 + 1] = M, l[d * 4 + 2] = A, l[d * 4 + 3] = x;
        var R = a[v + 1], C = -a[v], F = R * a[p] + C * a[p + 1];
        F < 0 && (R *= -1, C *= -1);
        var z = E + R * h, I = M + C * h, X = Math.pow(z - e, 2) + Math.pow(I - r, 2);
        if (X <= c)
          return !0;
      }
      return Yt(e, r, l);
      ...
      if (m !== 0) {
        var b = y / m, E = g / m, M = 1e-3, A = 0 - M, x = 1 + M;
        return A <= b && b <= x && A <= E && E <= x ? [e + b * h, r + b * v] : u ? [e + b * h, r + b * v] : [];
      } else
        return y === 0 || g === 0 ? ci(e, a, o) === o ? [o, l] : ci(e, a, i) === i ? [i, s] : ci(i, o, a) === a ? [a, n] : [] : [];
      ...
      for (var y, g, m, b, E = 0; E < d.length / 2; E++)
        y = d[E * 2], g = d[E * 2 + 1], E < d.length / 2 - 1 ? (m = d[(E + 1) * 2], b = d[(E + 1) * 2 + 1]) : (m = d[0], b = d[1]), f = Tr(e, r, n, i, y, g, m, b), f.length !== 0 && u.push(f[0], f[1]);
      return u;
    }, Bf = function(e, r, a, n, i, s, o, l) {
      for (var u = [], f, h = new Array(a.length), c = s / 2, d = o / 2, v = di(s, o), p = 0; p < a.length / 4; p++) {
        var y = void 0, g = void 0;
        p === 0 ? y = a.length - 2 : y = p * 4 - 2, g = p * 4 + 2;
        var m = n + c * a[p * 4], b = i + d * a[p * 4 + 1], E = -a[y] * a[g] - a[y + 1] * a[g + 1], M = v / Math.tan(Math.acos(E) / 2), A = m - M * a[y], x = b - M * a[y + 1], R = m + M * a[g], C = b + M * a[g + 1];
        p === 0 ? (h[a.length - 2] = A, h[a.length - 1] = x) : (h[p * 4 - 2] = A, h[p * 4 - 1] = x), h[p * 4] = R, h[p * 4 + 1] = C;
        var F = a[y + 1], z = -a[y], I = F * a[g] + z * a[g + 1];
        I < 0 && (F *= -1, z *= -1);
        var X = A + F * v, B = x + z * v;
        f = Pa(e, r, n, i, X, B, v), f.length !== 0 && u.push(f[0], f[1]);
      }
      ...
      if (u.length > 2) {
        for (var W = [u[0], u[1]], Z = Math.pow(W[0] - e, 2) + Math.pow(W[1] - r, 2), ie = 1; ie < u.length / 2; ie++) {
          ...
          ue <= Z && (W[0] = u[ie * 2], W[1] = u[ie * 2 + 1], Z = ue);
        }
        return W;
      }
      return u;
      ...
      pageRank: function(e) {
        for (var r = zf(e), a = r.dampingFactor, n = r.precision, i = r.iterations, s = r.weight, o = this._private.cy, l = this.byGroup(), u = l.nodes, f = l.edges, h = u.length, c = h * h, d = f.length, v = new Array(c), p = new Array(h), y = (1 - a) / h, g = 0; g < h; g++) {
          for (var m = 0; m < h; m++) {
            var b = g * h + m;
            v[b] = 0;
          }
        ...
        for (var E = 0; E < d; E++) {
          var M = f[E], A = M.data("source"), x = M.data("target");
          if (A !== x) {
            var R = u.indexOfId(A), C = u.indexOfId(x), F = s(M), z = C * h + R;
            v[z] += F, p[R] += F;
          }
        }
        for (var I = 1 / h + y, X = 0; X < h; X++)
          if (p[X] === 0)
            for (var B = 0; B < h; B++) {
              var re = B * h + X;
              v[re] = I;
        ...
        for (var ie = new Array(h), ue = new Array(h), pe, se = 0; se < h; se++)
          ie[se] = 1;
    ...
    var $f = Lt({
      ...
      betweennessCentrality: function(e) {
        ...
        for (var p = function(b) {
          for (var E = o[b].id(), M = [], A = {}, x = {}, R = {}, C = new Ra(function(Ee, ye) {
            return R[Ee] - R[ye];
          }), F = 0; F < o.length; F++) {
            var z = o[F].id();
            A[z] = [], x[z] = 0, R[z] = 1 / 0;
          }
          for (x[E] = 1, R[E] = 0, C.push(E); !C.empty(); ) {
            var I = C.pop();
            if (M.push(I), i)
              for (var X = 0; X < l[I].length; X++) {
                var B = l[I][X], re = s.getElementById(I), W = void 0;
                re.edgesTo(B).length > 0 ? W = re.edgesTo(B)[0] : W = B.edgesTo(re)[0];
                var Z = n(W);
                B = B.id(), R[B] > R[I] + Z && (R[B] = R[I] + Z, C.nodes.indexOf(B) < 0 ? C.push(B) : C.updateItem(B), x[B] = 0, A[B] = []), R[B] == R[I] + Z && (x[B] = x[B] + x[I], A[B].push(I));
              }
            else
              for (var ie = 0; ie < l[I].length; ie++) {
                var ue = l[I][ie].id();
                R[ue] == 1 / 0 && (C.push(ue), R[ue] = R[I] + 1), R[ue] == R[I] + 1 && (x[ue] = x[ue] + x[I], A[ue].push(I));
              }
          }
          for (var pe = {}, se = 0; se < o.length; se++)
            pe[o[se].id()] = 0;
          for (; M.length > 0; ) {
            for (var de = M.pop(), we = 0; we < A[de].length; we++) {
              var Te = A[de][we];
              pe[Te] = pe[Te] + x[Te] / x[de] * (1 + pe[de]);
            }
            de != o[b].id() && h.set(de, h.get(de) + pe[de]);
          }
        }, y = 0; y < o.length; y++)
          p(y);
        var g = {
    ...
    var Vf = Lt({
      ...
      for (var d = 0; d < a.length; d++) {
        var v = a[d], p = s[v.source().id()], y = s[v.target().id()], g = Uf(v, i.attributes);
        f[p * l + y] += g, f[y * l + p] += g;
      }
    ...
    var rh = Lt({
        ...
        for (var c = 0; c < i.k; c++) {
          ...
          if (d.length !== 0) {
            for (var v = i.attributes.length, p = l[c], y = new Array(v), g = new Array(v), m = 0; m < v; m++) {
              ...
              y[m] = g[m] / d.length, ah(y[m], p[m], i.sensitivityThreshold) || (u = !0);
            }
      ...
      for (var h = !0, c = 0; h && c < i.maxIterations; ) {
        for (var d = 0; d < a.length; d++)
          n = a[d], l[n.id()] = Ps(n, o, i.distance, i.attributes, "kMedoids");
        h = !1;
        for (var v = 0; v < o.length; v++) {
          var p = Bs(v, a, l);
          if (p.length !== 0) {
            f[v] = zs(o[v], p, i.attributes);
            for (var y = 0; y < p.length; y++)
              u = zs(p[y], p, i.attributes), u < f[v] && (f[v] = u, o[v] = p[y], h = !0);
            s[v] = r.collection(p);
          }
        }
        c++;
      ...
      for (var o, l, u, f = 2 / (i.m - 1), h = 0; h < a.length; h++)
        for (var c = 0; c < n.length; c++) {
          o = 0;
          for (var d = 0; d < a.length; d++)
            l = bn(i.distance, n[c], a[h], i.attributes, "cmeans"), u = bn(i.distance, n[c], a[d], i.attributes, "cmeans"), o += Math.pow(l / u, f);
          e[c][h] = 1 / o;
        }
    }, fh = function(e, r, a, n) {
      ...
      for (var s = 0, o = 1 / 0, l, u = i.attributes, f = function(C, F) {
        return mn(i.distance, u.length, function(z) {
          return u[z](C);
        }, function(z) {
          return u[z](F);
        }, C, F);
      ...
      for (var y = $s(o, f, l, u, n); y; )
        y = $s(o, f, l, u, n);
      var g;
      return n.mode === "dendrogram" ? (g = ph(o[0], n.dendrogramDepth, r), n.addDendrogram && gh(o[0], r)) : (g = new Array(o.length), o.forEach(function(m, b) {
      ...
      for (var v = 0; v < o; v++)
        for (var p = 0; p < o; p++)
          v !== p && (u[v * o + p] = Eh(n.distance, a[v], a[p], n.attributes));
      f = wh(u, n.preference);
      for (var y = 0; y < o; y++)
        u[y * o + y] = f;
      h = new Array(l);
      ...
      for (var b = new Array(o), E = new Array(o), M = new Array(o), A = 0; A < o; A++)
        b[A] = 0, E[A] = 0, M[A] = 0;
      for (var x = new Array(o * n.minIterations), R = 0; R < x.length; R++)
        x[R] = 0;
      var C;
      for (C = 0; C < n.maxIterations; C++) {
        for (var F = 0; F < o; F++) {
          for (var z = -1 / 0, I = -1 / 0, X = -1, B = 0, re = 0; re < o; re++)
            b[re] = h[F * o + re], B = c[F * o + re] + u[F * o + re], B >= z ? (I = z, z = B, X = re) : B > I && (I = B);
          for (var W = 0; W < o; W++)
            h[F * o + W] = (1 - n.damping) * (u[F * o + W] - z) + n.damping * b[W];
          h[F * o + X] = (1 - n.damping) * (u[F * o + X] - I) + n.damping * b[X];
        }
        for (var Z = 0; Z < o; Z++) {
          ...
          for (var pe = 0; pe < o; pe++)
            c[pe * o + Z] = (1 - n.damping) * Math.min(0, ie - E[pe]) + n.damping * b[pe];
      ...
      hierholzer: function(e) {
        ...
        i ? s.forEach(function(m) {
          var b = m.id();
          if (m.isNode()) {
            var E = m.indegree(!0), M = m.outdegree(!0), A = E - M, x = M - E;
            A == 1 ? l ? o = !0 : l = b : x == 1 ? u ? o = !0 : u = b : (x > 1 || A > 1) && (o = !0), h[b] = [], m.outgoers().forEach(function(R) {
              R.isEdge() && h[b].push(R.id());
            });
          } else
        ...
        if (u && l)
          ...
          f || (f = s[0].id());
        var v = function(b) {
          for (var E = b, M = [b], A, x, R; h[E].length; )
            A = h[E].shift(), x = c[A][0], R = c[A][1], E != R ? (h[R] = h[R].filter(function(C) {
              return C != A;
            }), E = R) : !i && E != x && (h[x] = h[x].filter(function(C) {
              return C != A;
            }), E = x), M.unshift(A), M.unshift(E);
          return M;
        }, p = [], y = [];
    ...
    */
        ...
        return e = Ue({}, r, e), function(n, i) {
          ...
          if (j(n)) {
            ...
            if (s.allowGetting && i === void 0) {
              var d;
              return f && (s.beforeGet(f), c && f._private[s.field][n] === void 0 ? d = mv(f._private[s.field], c) : d = f._private[s.field][n]), d;
            ...
            var I;
            return f && (s.beforeGet(f), I = f._private[s.field]), I;
          }
          return o;
        };
      },
      ...
      for (var e = function(f) {
        ...
        switch (c) {
          ...
          case Be.COMPOUND_SPLIT: {
            var A = i(f.left, h), x = i(f.subject, h), R = i(f.right, h);
            return A + (A.length > 0 ? " " : "") + x + R;
          }
          case Be.TRUE:
            return "";
        }
    ...
    var Kt = function(e, r) {
      return function(n, i, s, o) {
        ...
        if (l == null ? f = "" : ge(l) && l.length === 1 && (f = l.id()), u.length === 1 && f) {
          var h = u[0]._private, c = h.traversalCache = h.traversalCache || {}, d = c[r] = c[r] || [], v = kr(f), p = d[v];
    ...
    ca = Ar = {}, Ar.renderedBoundingBox = function(t) {
      ...
      function r(s) {
        ...
        function d(C, F, z) {
          var I = 0, X = 0, B = F + z;
          return C > 0 && B > 0 && (I = F / B * C, X = z / B * C), {
            biasDiff: I,
        ...
        var b = d(f.width.val - h.w, p, y), E = b.biasDiff, M = b.biasComplementDiff, A = d(f.height.val - h.h, g, m), x = A.biasDiff, R = A.biasComplementDiff;
        o.autoPadding = v(h.w, h.h, s.pstyle("padding"), s.pstyle("padding-relative-to").value), o.autoWidth = Math.max(h.w, f.width.val), c.x = (-E + h.x1 + h.x2 + M) / 2, o.autoHeight = Math.max(h.h, f.height.val), c.y = (-x + h.y1 + h.y2 + R) / 2;
      }
      for (var a = 0; a < this.length; a++) {
    ...
    var Zt = function(e) {
      ...
      if (!r.cy().headless()) {
        ...
        if (o) {
          var l = r.pstyle("text-halign"), u = r.pstyle("text-valign"), f = Va(s, "labelWidth", a), h = Va(s, "labelHeight", a), c = Va(s, "labelX", a), d = Va(s, "labelY", a), v = r.pstyle(n + "text-margin-x").pfValue, p = r.pstyle(n + "text-margin-y").pfValue, y = r.isEdge(), g = r.pstyle(n + "text-rotation"), m = r.pstyle("text-outline-width").pfValue, b = r.pstyle("text-border-width").pfValue, E = b / 2, M = r.pstyle("text-background-padding").pfValue, A = 2, x = h, R = f, C = R / 2, F = x / 2, z, I, X, B;
          if (y)
            z = c - C, I = c + C, X = d - F, B = d + F;
          else {
            switch (l.value) {
              case "left":
                z = c - R, I = c;
          ...
          z += v - Math.max(m, E) - M - A, I += v + Math.max(m, E) + M + A, X += p - Math.max(m, E) - M - A, B += p + Math.max(m, E) + M + A;
          var re = a || "main", W = i.labelBounds, Z = W[re] = W[re] || {};
          Z.x1 = z, Z.y1 = X, Z.x2 = I, Z.y2 = B, Z.w = I - z, Z.h = B - X;
          var ie = y && g.strValue === "autorotate", ue = g.pfValue != null && g.pfValue !== 0;
          if (ie || ue) {
            var pe = ie ? Va(i.rstyle, "labelAngle", a) : g.pfValue, se = Math.cos(pe), de = Math.sin(pe), we = (z + I) / 2, Te = (X + B) / 2;
            if (!y) {
              switch (l.value) {
                case "left":
                  we = I;
            ...
            }, ye = Ee(z, X), ae = Ee(z, B), xe = Ee(I, X), Ce = Ee(I, B);
            z = Math.min(ye.x, ae.x, xe.x, Ce.x), I = Math.max(ye.x, ae.x, xe.x, Ce.x), X = Math.min(ye.y, ae.y, xe.y, Ce.y), B = Math.max(ye.y, ae.y, xe.y, Ce.y);
          }
          var Oe = re + "Rot", Ie = W[Oe] = W[Oe] || {};
          Ie.x1 = z, Ie.y1 = X, Ie.x2 = I, Ie.y2 = B, Ie.w = I - z, Ie.h = B - X, rr(e, z, X, I, B), rr(i.labelBounds.all, z, X, I, B);
        }
        return e;
      }
    }, nd = function(e, r) {
      var a = e._private.cy, n = a.styleEnabled(), i = a.headless(), s = Ut(), o = e._private, l = e.isNode(), u = e.isEdge(), f, h, c, d, v, p, y = o.rstyle, g = l && n ? e.pstyle("bounds-expansion").pfValue : [0], m = function(Ve) {
      ...
      if (b) {
        ...
        var R = Math.max(M, x), C = 0, F = 0;
        if (n && (C = e.pstyle("width").pfValue, F = C / 2), l && r.includeNodes) {
          var z = e.position();
          v = z.x, p = z.y;
          var I = e.outerWidth(), X = I / 2, B = e.outerHeight(), re = B / 2;
          f = v - X, h = v + X, c = p - re, d = p + re, rr(s, f, c, h, d);
        } else if (u && r.includeEdges)
          if (n && !i) {
            var W = e.pstyle("curve-style").strValue;
            if (f = Math.min(y.srcX, y.midX, y.tgtX), h = Math.max(y.srcX, y.midX, y.tgtX), c = Math.min(y.srcY, y.midY, y.tgtY), d = Math.max(y.srcY, y.midY, y.tgtY), f -= F, h += F, c -= F, d += F, rr(s, f, c, h, d), W === "haystack") {
              var Z = y.haystackPts;
              if (Z && Z.length === 2) {
              ...
              if (pe != null)
                for (var se = 0; se < pe.length; se++) {
                  var de = pe[se];
                  f = de.x - F, h = de.x + F, c = de.y - F, d = de.y + F, rr(s, f, c, h, d);
                }
            }
          } else {
            var we = e.source(), Te = we.position(), Ee = e.target(), ye = Ee.position();
            if (f = Te.x, h = ye.x, c = Te.y, d = ye.y, f > h) {
              var ae = f;
              f = h, h = ae;
            }
            if (c > d) {
        ...
        ws(He, s), fi(He, g), pn(He, 1), n && (f = s.x1, h = s.x2, c = s.y1, d = s.y2, rr(s, f - R, c - R, h + R, d + R));
        var qe = o.overlayBounds = o.overlayBounds || {};
        ws(qe, s), fi(qe, g), pn(qe, 1);
      ...
      return s.x1 = Zt(s.x1), s.y1 = Zt(s.y1), s.x2 = Zt(s.x2), s.y2 = Zt(s.y2), s.w = Zt(s.x2 - s.x1), s.h = Zt(s.y2 - s.y1), s.w > 0 && s.h > 0 && b && (fi(s, g), pn(s, 1)), s;
    }, mo = function(e) {
      ...
      var a = e._private, n, i = e.isEdge(), s = r == null ? wo : mo(r), o = s === wo, l = bo(e), u = a.bbCachePosKey === l, f = r.useCache && u, h = function(p) {
      ...
      return this.emitting++, te(e) || (e = [e]), Ed(this, function(i, s) {
        ...
        for (var o = function(f) {
          ...
          if (h.type === s.type && (!h.namespace || h.namespace === s.namespace || h.namespace === md) && i.eventMatches(i.context, h, s)) {
            var c = [s];
            e != null && Ql(c, e), i.beforeEmit(i.context, h, s), h.conf && h.conf.one && (i.listeners = i.listeners.filter(function(p) {
              return p !== h;
            }));
    ...
    var Cd = Lt({
      ...
      layoutPositions: function(e, r, a) {
        ...
        var u = function(A, x, R) {
          ...
          if (!f)
            return null;
          for (var A = Ut(), x = 0; x < n.length; x++) {
            var R = n[x], C = l(R, x);
            Sf(A, C.x, C.y);
          }
          return A;
        }, c = h(), d = nt(function(M, A) {
          var x = l(M, A);
          if (f) {
            var R = Math.abs(r.spacingFactor);
            x = u(R, c, x);
          }
    ...
    mt.instanceString = function() {
      ...
      for (h = 0; h < o.length; h++) {
        ...
        if (d.clearTraversalCache(), !(!e && !v.removed)) {
          if (p.id === void 0)
            ...
            xt("Can not create element with invalid string ID `" + p.id + "`"), c();
            continue;
          } else if (a.hasElementWithId(p.id)) {
            xt("Can not create second element with ID `" + p.id + "`"), c();
            continue;
          }
        }
        var y = p.id;
        ...
        if (d.isEdge()) {
          for (var m = d, b = ["source", "target"], E = b.length, M = !1, A = 0; A < E; A++) {
            var x = b[A], R = p[x];
            k(R) && (R = p[x] = "" + p[x]), R == null || R === "" ? (xt("Can not create edge `" + y + "` with unspecified " + x), M = !0) : a.hasElementWithId(R) || (xt("Can not create edge `" + y + "` with nonexistant " + x + " `" + R + "`"), M = !0);
          }
          if (M) {
            c();
            continue;
        ...
        v.map = new ur(), v.map.set(y, {
          ele: d,
          index: 0
        }), v.removed = !1, e && a.addToPool(d);
      }
      for (var z = 0; z < i.length; z++) {
        var I = i[z], X = I._private.data;
        k(X.parent) && (X.parent = "" + X.parent);
        var B = X.parent, re = B != null;
        if (re || I._private.parent) {
          var W = I._private.parent ? a.collection().merge(I._private.parent) : a.getElementById(B);
          if (W.empty())
            X.parent = void 0;
          else if (W[0].removed())
            ft("Node added with missing parent, reference to parent removed"), X.parent = void 0, I._private.parent = null;
          else {
            ...
            Z || (W[0]._private.children.push(I), I._private.parent = W[0], n.hasCompoundNodes = !0);
          }
        }
      }
      if (o.length > 0) {
        for (var ue = o.length === r.length ? r : new Ot(a, o), pe = 0; pe < ue.length; pe++) {
          var se = ue[pe];
          se.isNode() || (se.parallelEdges().clearTraversalCache(), se.source().clearTraversalCache(), se.target().clearTraversalCache());
        }
        var de;
        n.hasCompoundNodes ? de = a.collection().merge(ue).merge(ue.connectedNodes()).merge(ue.parent()) : de = ue, de.dirtyCompoundBoundsCache().dirtyBoundingBoxCache().updateStyle(t), t ? ue.emitAndNotify("add") : e && ue.emit("add");
      }
      return r;
    }, mt.removed = function() {
      var t = this[0];
      ...
      function s(B) {
        for (var re = B._private.edges, W = 0; W < re.length; W++)
          l(re[W]);
      }
      function o(B) {
        for (var re = B._private.children, W = 0; W < re.length; W++)
          l(re[W]);
      }
      function l(B) {
        var re = n[B.id()];
      ...
      function c(B, re) {
        var W = B._private.edges;
        wr(W, re), B.clearTraversalCache();
      }
      function d(B) {
      ...
      function p(B, re) {
        re = re[0], B = B[0];
        var W = B._private.children, Z = B.id();
        wr(W, re), re._private.parent = null, v.ids[Z] || (v.ids[Z] = !0, v.push(B));
      }
    ...
    function Nd(t, e, r, a) {
      ...
      function y(F, z, I) {
        return ((d(z, I) * F + v(z, I)) * F + p(z)) * F;
      }
      function g(F, z, I) {
        return 3 * d(z, I) * F * F + 2 * v(z, I) * F + p(z);
      }
      function m(F, z) {
        for (var I = 0; I < n; ++I) {
          var X = g(z, t, r);
      ...
      function E(F, z, I) {
        var X, B, re = 0;
        do
          B = z + (I - z) / 2, X = y(B, t, r) - F, X > 0 ? I = B : z = B;
        while (Math.abs(X) > s && ++re < o);
        return B;
      }
      function M(F) {
        for (var z = 0, I = 1, X = l - 1; I !== X && c[I] <= F; ++I)
          z += u;
        --I;
        var B = (F - c[I]) / (c[I + 1] - c[I]), re = z + B * u, W = g(re, t, r);
        return W >= i ? m(F, re) : W === 0 ? re : E(F, z, z + u);
      }
      var A = !1;
      function x() {
        A = !0, (t !== e || r !== a) && b();
      ...
      var R = function(z) {
        return A || x(), t === e && r === a ? z : z === 0 ? 0 : z === 1 ? 1 : y(M(z), e, a);
      };
      R.getControlPoints = function() {
        return [{
          x: t,
          y: e
        }, {
    ...
    function Id(t, e, r, a) {
      ...
      if (!s.easingImpl)
        if (o == null)
          ...
          j(h) ? (d = h, v = []) : (d = h[1], v = h.slice(2).map(function(ue) {
            return +ue;
          })), v.length > 0 ? (d === "spring" && v.push(s.duration), s.easingImpl = Bn[d].apply(null, v)) : s.easingImpl = Bn[d];
        }
      ...
      if (s.duration === 0 ? y = 1 : y = (r - l) / s.duration, s.applying && (y = s.progress), y < 0 ? y = 0 : y > 1 && (y = 1), s.delay == null) {
        ...
        if (m && n && !t.locked()) {
          var b = {};
          Xa(g.x, m.x) && (b.x = ga(g.x, m.x, y, p)), Xa(g.y, m.y) && (b.y = ga(g.y, m.y, y, p)), t.position(b);
        }
        var E = s.startPan, M = s.pan, A = i.pan, x = M != null && a;
        x && (Xa(E.x, M.x) && (A.x = ga(E.x, M.x, y, p)), Xa(E.y, M.y) && (A.y = ga(E.y, M.y, y, p)), t.emit("pan"));
        var R = s.startZoom, C = s.zoom, F = C != null && a;
        F && (Xa(R, C) && (i.zoom = ka(i.minZoom, ga(R, C, y, p), i.maxZoom)), t.emit("zoom")), (x || F) && t.emit("viewport");
        var z = s.style;
        if (z && z.length > 0 && n) {
          for (var I = 0; I < z.length; I++) {
            var X = z[I], B = X.name, re = X, W = s.startStyle[B], Z = f.properties[W.name], ie = ga(W, re, y, p, Z);
    ...
    function Yo(t, e) {
      ...
      function n(f, h) {
        ...
        for (var g = function(A) {
          ...
          !E.playing && !E.applying || (E.playing && E.applying && (E.applying = !1), E.started || Rd(f, b, t), Id(f, b, t, h), E.applying && (E.applying = !1), g(E.frames), E.step != null && E.step(t), b.completed() && (d.splice(m, 1), E.hooked = !1, E.playing = !1, E.started = !1, g(E.completes)), p = !0);
        }
    ...
    Bt.apply = function(t) {
      ...
      for (var h = function(Oe, Ie) {
        ...
        var m = a[g], b = l[m];
        if (b != null) {
          var E = this.properties[m], M = E.type, A = E.groupKey, x = void 0;
          E.hashOverride != null ? x = E.hashOverride(t, b) : b.pfValue != null && (x = b.pfValue);
          var R = E.enums == null ? b.value : null, C = x != null, F = R != null, z = C || F, I = b.units;
          if (M.number && z && !M.multiple) {
            var X = C ? x : R;
            d(y(X), A), !C && I != null && v(I, A);
          } else
            v(b.strValue, A);
        }
      }
      for (var B = [Qr, Oa], re = 0; re < n.length; re++) {
        ...
        B[0] = Na(Z[0], B[0]), B[1] = Ma(Z[1], B[1]);
      }
      e.styleKey = Ul(B[0], B[1]);
      var ie = e.styleKeys;
      e.labelDimsKey = Er(ie.labelDimensions);
      var ue = i(t, ["label"], ie.labelDimensions);
      if (e.labelKey = Er(ue), e.labelStyleKey = Er(hn(ie.commonLabel, ue)), !o) {
        var pe = i(t, ["source-label"], ie.labelDimensions);
        e.sourceLabelKey = Er(pe), e.sourceLabelStyleKey = Er(hn(ie.commonLabel, pe));
      ...
      var r = this, a = e, n = t._private.style, i, s = r.types, o = r.properties[a.name].type, l = a.bypass, u = n[a.name], f = u && u.bypass, h = t._private, c = "mapping", d = function(we) {
      ...
      switch (a.mapped) {
        case s.mapData: {
          ...
          if (E < 0 ? E = 0 : E > 1 && (E = 1), o.color) {
            var A = a.valueMin[0], x = a.valueMax[0], R = a.valueMin[1], C = a.valueMax[1], F = a.valueMin[2], z = a.valueMax[2], I = a.valueMin[3] == null ? 1 : a.valueMin[3], X = a.valueMax[3] == null ? 1 : a.valueMax[3], B = [Math.round(A + (x - A) * E), Math.round(R + (C - R) * E), Math.round(F + (z - F) * E), Math.round(I + (X - I) * E)];
            i = {
        ...
        case s.fn: {
          var pe = a.value, se = a.fnValue != null ? a.fnValue : pe(t);
      ...
      if (n.length > 0 && i > 0) {
        for (var o = {}, l = !1, u = 0; u < n.length; u++) {
          ...
          if (c) {
            ...
            v && (k(v.pfValue) && k(p.pfValue) ? (y = p.pfValue - v.pfValue, g = v.pfValue + m * y) : k(v.value) && k(p.value) ? (y = p.value - v.value, g = v.value + m * y) : te(v.value) && te(p.value) && (y = v.value[0] !== p.value[0] || v.value[1] !== p.value[1] || v.value[2] !== p.value[2], g = v.strValue), y && (o[f] = p.strValue, this.applyBypass(t, f, g), l = !0));
          }
        }
        if (!l)
          return;
    ...
    Wa.applyBypass = function(t, e, r, a) {
      ...
      for (var m = !1, b = 0; b < t.length; b++) {
        for (var E = t[b], M = {}, A = void 0, x = 0; x < i.length; x++) {
          var R = i[x];
          if (a) {
            var C = E.pstyle(R.name);
            A = M[R.name] = {
              prev: C
            };
          }
          m = this.applyParsedProperty(E, or(R)) || m, a && (A.next = E.pstyle(R.name));
        }
    ...
    (function() {
      ...
      var B = [], re = Nt.arrowPrefixes = ["source", "mid-source", "target", "mid-target"];
      [{
        name: "arrow-shape",
        type: u.arrowShape,
        triggersBounds: l.any
      ...
      var W = Nt.properties = [].concat(p, b, y, g, m, F, v, d, f, h, c, M, A, x, I, R, C, B, z), Z = Nt.propertyGroups = {
        // common to all eles
        behavior: p,
        transition: b,
        visibility: y,
        ...
        edgeArrow: B,
        core: z
      }, ie = Nt.propertyGroupNames = {}, ue = Nt.propertyGroupKeys = Object.keys(Z);
      ...
      for (var we = 0; we < pe.length; we++) {
        var Te = pe[we], Ee = W[Te.pointsTo], ye = {
          name: Te.name,
          alias: !0,
          pointsTo: Ee
        };
    ...
    Vn.parse = function(t, e, r, a) {
      ...
      if (u.multiple && a !== "multiple") {
        ...
        if (l ? b = e.split(/\s+/) : te(e) ? b = e : b = [e], u.evenMultiple && b.length % 2 !== 0)
          return null;
        for (var E = [], M = [], A = [], x = "", R = !1, C = 0; C < b.length; C++) {
          var F = n.parse(t, b[C], r, "multiple");
          R = R || j(F.value), E.push(F.value), A.push(F.pfValue != null ? F.pfValue : F.value), M.push(F.units), x += (C > 0 ? " " : "") + F.strValue;
        }
        return u.validate && !u.validate(E, M) ? null : u.singleEnum && R ? E.length === 1 && j(E[0]) ? {
          name: t,
          value: E[0],
          strValue: E[0],
          bypass: r
      ...
      if (u.number) {
        var I, X = "px";
        if (u.units && (I = u.units), u.implicitUnits && (X = u.implicitUnits), !u.unitless)
          if (l) {
        ...
        return u.unitless || I !== "px" && I !== "em" ? W.pfValue = e : W.pfValue = I === "px" || !I ? e : this.getEmSizeInPixels() * e, (I === "ms" || I === "s") && (W.pfValue = I === "ms" ? e : 1e3 * e), (I === "deg" || I === "rad") && (W.pfValue = I === "rad" ? e : Ef(e)), I === "%" && (W.pfValue = e / 100), W;
      } else if (u.propList) {
        var Z = [], ie = "" + e;
        if (ie !== "none") {
          for (var ue = ie.split(/\s*,\s*|\s+/), pe = 0; pe < ue.length; pe++) {
            var se = ue[pe].trim();
    ...
    Ue(_n, {
      ...
      json: function(e) {
        ...
        if (L(e)) {
          ...
          })), this._private.styleEnabled && (g.style = r.style().json()), g.data = or(r.data());
          var m = a.options;
    ...
    function qo(t) {
      this.options = Ue({}, Vd, _d, t);
    }
    qo.prototype.run = function() {
      var t = this.options, e = t, r = t.cy, a = e.eles, n = a.nodes().filter(function(Re) {
        return !Re.isParent();
      }), i = a, s = e.directed, o = e.acyclic || e.maximal || e.maximalAdjustments > 0, l = Ut(e.boundingBox ? e.boundingBox : {
        x1: 0,
        y1: 0,
        w: r.width(),
        h: r.height()
      ...
      if (ge(e.roots))
        ...
        for (var p = function(Me) {
          var Ve = v[Me], Fe = Ve.maxDegree(!1), ke = Ve.filter(function(Ge) {
      ...
      var R = function(Me) {
        for (var Ve = g[Me], Fe = 0; Fe < Ve.length; Fe++) {
          var ke = Ve[Fe];
          if (ke == null) {
            Ve.splice(Fe, 1), Fe--;
            continue;
        ...
        for (var Fe = pa(Me), ke = Me.incomers().filter(function(le) {
        ...
        if (Fe.depth <= Ge) {
          if (!e.acyclic && Ve[et])
            return null;
          var N = Ge + 1;
          return E(Me, N), Ve[et] = N, !0;
        }
        return !1;
      };
      if (s && o) {
        var z = [], I = {}, X = function(Me) {
          return z.push(Me);
        }, B = function() {
          return z.shift();
        };
      ...
      if (e.avoidOverlap)
        for (var ie = 0; ie < n.length; ie++) {
          var ue = n[ie], pe = ue.layoutDimensions(e), se = pe.w, de = pe.h;
          Z = Math.max(Z, se, de);
        }
      ...
      var Ie = {
        x: l.x1 + l.w / 2,
        y: l.x1 + l.h / 2
      }, He = g.reduce(function(Re, Me) {
        return Math.max(Re, Me.length);
      }, 0), qe = function(Me) {
        var Ve = pa(Me), Fe = Ve.depth, ke = Ve.index, Ge = g[Fe].length, et = Math.max(l.w / ((e.grid ? He : Ge) + 1), Z), We = Math.max(l.h / (g.length + 1), Z), Ye = Math.min(l.w / 2 / g.length, l.h / 2 / g.length);
        if (Ye = Math.max(Ye, Z), e.circle) {
          var N = Ye * Fe + Ye - (g.length > 0 && g[0].length <= 3 ? Ye / 2 : 0), le = 2 * Math.PI / g[Fe].length * ke;
          return Fe === 0 && g[0].length === 1 && (N = 1), {
    ...
    Wo.prototype.run = function() {
      ...
      var E = function(A, x) {
        var R = e.startAngle + x * u * (n ? 1 : -1), C = f * Math.cos(R), F = f * Math.sin(R), z = {
          x: o.x + C,
          y: o.y + F
        };
        return z;
    ...
    Ko.prototype.run = function() {
      ...
      if (!e.avoidOverlap) {
        var x = g.length > 0 && g[0].length > 1, R = Math.min(s.w, s.h) / 2 - A, C = R / (g.length + x ? 1 : 0);
        A = Math.min(A, C);
      }
      for (var F = 0, z = 0; z < g.length; z++) {
        var I = g[z], X = e.sweep === void 0 ? 2 * Math.PI - 2 * Math.PI / I.length : e.sweep, B = I.dTheta = X / Math.max(1, I.length - 1);
        if (I.length > 1 && e.avoidOverlap) {
          var re = Math.cos(B) - Math.cos(0), W = Math.sin(B) - Math.sin(0), Z = Math.sqrt(A * A / (re * re + W * W));
          F = Math.max(Z, F);
        }
        I.r = F, F += A;
      }
      if (e.equidistant) {
        for (var ie = 0, ue = 0, pe = 0; pe < g.length; pe++) {
          var se = g[pe], de = se.r - ue;
          ie = Math.max(ie, de);
        }
        ue = 0;
        for (var we = 0; we < g.length; we++) {
          var Te = g[we];
          we === 0 && (ue = Te.r), Te.r = ue, ue += ie;
        }
      }
      for (var Ee = {}, ye = 0; ye < g.length; ye++)
        for (var ae = g[ye], xe = ae.dTheta, Ce = ae.r, Oe = 0; Oe < ae.length; Oe++) {
      ...
      if (t.animate === !0) {
        var f = function h() {
          ...
          if (!u)
            ...
            var d = dr();
            d - n >= t.animationThreshold && i(), fn(h);
          }
        };
        f();
      } else {
    ...
    var Xd = function(e, r, a) {
      for (var n = a.eles.edges(), i = a.eles.nodes(), s = Ut(a.boundingBox ? a.boundingBox : {
        x1: 0,
        y1: 0,
        w: e.width(),
        h: e.height()
      ...
      for (o.graphSet.push(E); m <= b; ) {
        var A = g[m++], x = o.idToIndex[A], d = o.layoutNodes[x], R = d.children;
        if (R.length > 0) {
          o.graphSet.push(R);
      ...
      for (var f = 0; f < o.edgeSize; f++) {
        var z = n[f], I = {};
        I.id = z.data("id"), I.sourceId = z.data("source"), I.targetId = z.data("target");
        var X = Y(a.idealEdgeLength) ? a.idealEdgeLength(z) : a.idealEdgeLength, B = Y(a.edgeElasticity) ? a.edgeElasticity(z) : a.edgeElasticity, re = o.idToIndex[I.sourceId], W = o.idToIndex[I.targetId], Z = o.indexToGraph[re], ie = o.indexToGraph[W];
        if (Z != ie) {
          for (var ue = qd(I.sourceId, I.targetId, o), pe = o.graphSet[ue], se = 0, y = o.layoutNodes[re]; pe.indexOf(y.id) === -1; )
            y = o.layoutNodes[o.idToIndex[y.parentId]], se++;
          for (y = o.layoutNodes[W]; pe.indexOf(y.id) === -1; )
            y = o.layoutNodes[o.idToIndex[y.parentId]], se++;
          X *= se * a.nestingFactor;
        }
        I.idealLength = X, I.elasticity = B, o.layoutEdges.push(I);
      }
      return o;
    }, qd = function(e, r, a) {
      var n = Wd(e, r, 0, a);
      ...
      if (!(i !== s && !a.isCompound)) {
        ...
        if (f > 0)
          ...
          var p = Yn(e, o, l), y = Yn(r, -1 * o, -1 * l), g = y.x - p.x, m = y.y - p.y, b = g * g + m * m, c = Math.sqrt(b), h = (e.nodeRepulsion + r.nodeRepulsion) / b, d = h * g / c, v = h * m / c;
      ...
      for (var a = 0; a < e.edgeSize; a++) {
        ...
        if (!(u === 0 && f === 0)) {
          var h = Yn(s, u, f), c = Yn(l, -1 * u, -1 * f), d = c.x - h.x, v = c.y - h.y, p = Math.sqrt(d * d + v * v), y = Math.pow(n.idealLength - p, 2) / n.elasticity;
          if (p !== 0)
            var g = y * d / p, m = y * v / p;
          else
            var g = 0, m = 0;
      ...
      if (i.h === 0 || i.w === 0)
        ...
        if (e.condense && (m = 0, b = 0), e.avoidOverlap)
          for (var E = 0; E < n.length; E++) {
            ...
            var x = M.layoutDimensions(e), R = e.avoidOverlapPadding, C = x.w + R, F = x.h + R;
            m = Math.max(m, C), b = Math.max(b, F);
          }
        for (var z = {}, I = function(Te, Ee) {
          return !!z["c-" + Te + "-" + Ee];
        }, X = function(Te, Ee) {
          z["c-" + Te + "-" + Ee] = !0;
        }, B = 0, re = 0, W = function() {
          ...
          var ue = n[ie], pe = e.position(ue);
          if (pe && (pe.row !== void 0 || pe.col !== void 0)) {
            var se = {
              row: pe.row,
              col: pe.col
            };
            if (se.col === void 0)
              for (se.col = 0; I(se.row, se.col); )
                se.col++;
            else if (se.row === void 0)
              for (se.row = 0; I(se.row, se.col); )
                se.row++;
        ...
        var de = function(Te, Ee) {
          ...
          if (xe)
            ...
            for (; I(B, re); )
              W();
            ye = re * m + m / 2 + i.x1, ae = B * b + b / 2 + i.y1, X(B, re), W();
          }
          return {
            x: ye,
            y: ae
    ...
    zi.arrowShapeWidth = 0.3, zi.registerArrowShapes = function() {
      var t = this.arrowShapes = {}, e = this, r = function(u, f, h, c, d, v, p) {
        ...
        j(f) && (f = t[f]), t[u] = Ue({
          ...
          collide: function(c, d, v, p, y, g) {
            var m = i(n(this.points, v + 2 * g, p, y)), b = Yt(c, d, m);
            return b;
          },
          roughCollide: r,
      ...
      o("none", {
        ...
        collide: function(u, f, h, c, d, v, p) {
          var y = i(n(this.points, h + 2 * p, c, d)), g = i(n(this.pointsTee, h + 2 * p, c, d)), m = Yt(u, f, y) || Yt(u, f, g);
          return m;
        },
        draw: function(u, f, h, c, d) {
        ...
        collide: function(u, f, h, c, d, v, p) {
          var y = i(n(this.points, h + 2 * p, c, d)), g = i(n(this.crossLinePts(h, v), h + 2 * p, c, d)), m = Yt(u, f, y) || Yt(u, f, g);
          return m;
        },
        draw: function(u, f, h, c, d) {
    ...
    Vr.projectIntoViewport = function(t, e) {
      ...
      var t = this.container, e = t.getBoundingClientRect(), r = D.getComputedStyle(t), a = function(g) {
      ...
      function g(x) {
        var R = x.outerWidth() + 2 * h, C = x.outerHeight() + 2 * h, F = R / 2, z = C / 2, I = x.position();
        if (I.x - F <= t && t <= I.x + F && I.y - z <= e && e <= I.y + z) {
          var X = i.nodeShapes[n.getNodeShape(x)];
          if (X.checkPoint(t, e, 0, R, C, I.x, I.y))
            return y(x, 0), !0;
        }
      }
      function m(x) {
        var R = x._private, C = R.rscratch, F = x.pstyle("width").pfValue, z = x.pstyle("arrow-scale").value, I = F / 2 + f, X = I * I, B = I * 2, ie = R.source, ue = R.target, re;
        if (C.edgeType === "segments" || C.edgeType === "straight" || C.edgeType === "haystack") {
          for (var W = C.allpts, Z = 0; Z + 3 < W.length; Z += 2)
            if (Af(t, e, W[Z], W[Z + 1], W[Z + 2], W[Z + 3], B) && X > (re = Rf(t, e, W[Z], W[Z + 1], W[Z + 2], W[Z + 3])))
              return y(x, re), !0;
          ...
          for (var W = C.allpts, Z = 0; Z + 5 < C.allpts.length; Z += 4)
            if (Of(t, e, W[Z], W[Z + 1], W[Z + 2], W[Z + 3], W[Z + 4], W[Z + 5], B) && X > (re = If(t, e, W[Z], W[Z + 1], W[Z + 2], W[Z + 3], W[Z + 4], W[Z + 5])))
              return y(x, re), !0;
        }
        for (var ie = ie || R.source, ue = ue || R.target, pe = n.getArrowWidth(F, z), se = [{
          name: "source",
          x: C.arrowStartX,
          y: C.arrowStartY,
          ...
          if (we.roughCollide(t, e, pe, de.angle, {
            x: de.x,
            y: de.y
          }, Te, f) && we.collide(t, e, pe, de.angle, {
            x: de.x,
            y: de.y
          }, Te, f))
            return y(x), !0;
      ...
      function E(x, R) {
        ...
        R ? z = R + "-" : z = "", x.boundingBox();
        var I = C.labelBounds[R || "main"], X = x.pstyle(z + "label").value, B = x.pstyle("text-events").strValue === "yes";
        if (!(!B || !X)) {
          var re = b(C.rscratch, "labelX", R), W = b(C.rscratch, "labelY", R), Z = b(C.rscratch, "labelAngle", R), ie = x.pstyle(z + "text-margin-x").pfValue, ue = x.pstyle(z + "text-margin-y").pfValue, pe = I.x1 - F - ie, se = I.x2 + F - ie, de = I.y1 - F - ue, we = I.y2 + F - ue;
          if (Z) {
            ...
            }, ae = ye(pe, de), xe = ye(pe, we), Ce = ye(se, de), Oe = ye(se, we), Ie = [
    ...
    Hn.calculateArrowAngles = function(t) {
      ...
      if (r ? (f = e.haystackPts[0], h = e.haystackPts[1], c = e.haystackPts[2], d = e.haystackPts[3]) : (f = e.arrowStartX, h = e.arrowStartY, c = e.arrowEndX, d = e.arrowEndY), g = e.midX, m = e.midY, i)
        ...
        var v = e.allpts, p = St(v[0], v[2], v[4], 0.1), y = St(v[1], v[3], v[5], 0.1);
        l = f - p, u = h - y;
      } else
        l = f - g, u = h - m;
      ...
      if (r && (g = (f + c) / 2, m = (h + d) / 2), l = c - f, u = d - h, i) {
        ...
        if (v.length / 2 % 2 === 0) {
          ...
          var b = v.length / 2 - 1, E = b - 2, M = b + 2;
          l = v[b] - v[E], u = v[b + 1] - v[E + 1];
        }
      } else if (n || s || o) {
        var v = e.allpts, A = e.ctrlpts, x, R, C, F;
        if (A.length / 2 % 2 === 0) {
          var z = v.length / 2 - 1, I = z + 2, X = I + 2;
          x = St(v[z], v[I], v[X], 0), R = St(v[z + 1], v[I + 1], v[X + 1], 0), C = St(v[z], v[I], v[X], 1e-4), F = St(v[z + 1], v[I + 1], v[X + 1], 1e-4);
        } else {
          var I = v.length / 2 - 1, z = I - 2, X = I + 2;
          x = St(v[z], v[I], v[X], 0.4999), R = St(v[z + 1], v[I + 1], v[X + 1], 0.4999), C = St(v[z], v[I], v[X], 0.5), F = St(v[z + 1], v[I + 1], v[X + 1], 0.5);
        }
        l = C - x, u = F - R;
      }
      if (e.midtgtArrowAngle = gn(l, u), e.midDispX = l, e.midDispY = u, l *= -1, u *= -1, i) {
        var v = e.allpts;
        if (v.length / 2 % 2 !== 0) {
          var b = v.length / 2 - 1, M = b + 2;
          l = -(v[M] - v[b]), u = -(v[M + 1] - v[b + 1]);
        }
      }
      if (e.midsrcArrowAngle = gn(l, u), i)
        l = c - e.segpts[e.segpts.length - 2], u = d - e.segpts[e.segpts.length - 1];
      else if (n || s || o || a) {
        var v = e.allpts, B = v.length, p = St(v[B - 6], v[B - 4], v[B - 2], 0.9), y = St(v[B - 5], v[B - 3], v[B - 1], 0.9);
        l = c - p, u = d - y;
      } else
        l = c - g, u = d - m;
    ...
    Vt.findHaystackPoints = function(t) {
      ...
      n.ctrlpts = [b.x, b.y - (1 + Math.pow(y, 1.12) / 100) * p * (v / 3 + 1) * M, b.x - (1 + Math.pow(y, 1.12) / 100) * p * (v / 3 + 1) * A, b.y];
      ...
      for (var g = 0; g < d; g++) {
        ...
        var M = b !== void 0 ? b : m, A = 1 - p, x = p, R = u === "node-position" ? o : l, C = {
          x: R.x1 * A + R.x2 * x,
          y: R.y1 * A + R.y2 * x
        };
      ...
      if (ae && !pe)
        if (W) {
          ...
          var Ve = Math.abs(ie) <= h / 2, Fe = Math.abs(z) <= v / 2;
          if (Ve) {
      ...
      if (r.edgeType === "bezier") {
        ...
        if (h || c || b) {
          ...
          }, I = u.intersectLine(a.x, a.y, i, s, z.x, z.y, 0);
          b ? (r.ctrlpts[0] = r.ctrlpts[0] + C.x * (g - m), r.ctrlpts[1] = r.ctrlpts[1] + C.y * (g - m)) : (r.ctrlpts[0] = I[0] + C.x * g, r.ctrlpts[1] = I[1] + C.y * g);
        }
        if (d || v || M) {
          A = !0;
          var X = {
          ...
          M ? (r.ctrlpts[0] = r.ctrlpts[0] + re.x * (g - E), r.ctrlpts[1] = r.ctrlpts[1] + re.y * (g - E)) : (r.ctrlpts[0] = ie[0] + re.x * g, r.ctrlpts[1] = ie[1] + re.y * g);
        }
      ...
      if (!(!t || t.length === 0)) {
        for (var r = this, a = r.cy, n = a.hasCompoundNodes(), i = {
          map: new ur(),
          get: function(x) {
            var R = this.map.get(x[0]);
            return R != null ? R.get(x[1]) : null;
          },
          set: function(x, R) {
            var C = this.map.get(x[0]);
            C == null && (C = new ur(), this.map.set(x[0], C)), C.set(x[1], R);
          }
        ...
        for (var E = function(x) {
          var R = s[x], C = i.get(R), F = void 0;
          if (!C.hasUnbundled) {
            var z = C.eles[0].parallelEdges().filter(function(Se) {
              return Se.isBundledBezier();
            });
            oi(C.eles), z.forEach(function(Se) {
              return C.eles.push(Se);
            }), C.eles.sort(function(Se, N) {
              return Se.poolIndex() - N.poolIndex();
            });
          }
          var I = C.eles[0], X = I.source(), B = I.target();
          if (X.poolIndex() > B.poolIndex()) {
            var re = X;
            X = B, B = re;
          }
          var W = C.srcPos = X.position(), Z = C.tgtPos = B.position(), ie = C.srcW = X.outerWidth(), ue = C.srcH = X.outerHeight(), pe = C.tgtW = B.outerWidth(), se = C.tgtH = B.outerHeight(), de = C.srcShape = r.nodeShapes[e.getNodeShape(X)], we = C.tgtShape = r.nodeShapes[e.getNodeShape(B)];
          C.dirCounts = {
            north: 0,
            west: 0,
            south: 0,
          ...
          for (var Te = 0; Te < C.eles.length; Te++) {
            ...
            if (!C.calculatedIntersection && X !== B && (C.hasBezier || C.hasUnbundled)) {
              ...
              var Oe = de.intersectLine(W.x, W.y, ie, ue, Z.x, Z.y, 0), Ie = C.srcIntn = Oe, He = we.intersectLine(Z.x, Z.y, pe, se, W.x, W.y, 0), qe = C.tgtIntn = He, Re = C.intersectionPts = {
                x1: Oe[0],
                x2: He[0],
                y1: Oe[1],
                y2: He[1]
              ...
              C.nodesOverlap = !k(ke) || we.checkPoint(Oe[0], Oe[1], 0, pe, se, Z.x, Z.y) || de.checkPoint(He[0], He[1], 0, ie, ue, W.x, W.y), C.vectorNormInverse = We, F = {
    ...
    Qa.manualEndptToPx = function(t, e) {
      ...
      if (C === "inside-to-node")
        ...
        r = this.manualEndptToPx(n, R);
      else if (C === "outside-to-line")
        r = c.tgtIntn;
      else if (C === "outside-to-node" || C === "outside-to-node-or-label" ? I = F : (C === "outside-to-line" || C === "outside-to-line-or-label") && (I = [i.x, i.y]), r = e.nodeShapes[this.getNodeShape(n)].intersectLine(s.x, s.y, n.outerWidth(), n.outerHeight(), I[0], I[1], 0), C === "outside-to-node-or-label" || C === "outside-to-line-or-label") {
        var ie = n._private.rscratch, ue = ie.labelWidth, pe = ie.labelHeight, se = ie.labelX, de = ie.labelY, we = ue / 2, Te = pe / 2, Ee = n.pstyle("text-valign").value;
        Ee === "top" ? de -= Te : Ee === "bottom" && (de += Te);
        var ye = n.pstyle("text-halign").value;
        ye === "left" ? se -= we : ye === "right" && (se += we);
        var ae = Ba(I[0], I[1], [se - we, de - Te, se + we, de - Te, se + we, de + Te, se - we, de + Te], s.x, s.y);
        if (ae.length > 0) {
      ...
      var qe = yn(r, F, e.arrowShapes[o].spacing(t) + u), Re = yn(r, F, e.arrowShapes[o].gap(t) + u);
      ...
      var Le = yn(r, z, e.arrowShapes[l].spacing(t) + f), ze = yn(r, z, e.arrowShapes[l].gap(t) + f);
    ...
    fr.recalculateEdgeLabelProjections = function(t) {
      ...
      if (i.mid || i.source || i.target) {
        ...
        var s = function(h, c, d) {
          xr(r.rscratch, h, c, d), xr(r.rstyle, h, c, d);
        };
        ...
        var l = function f() {
          ...
          function m(x, R, C, F, z) {
            var I = Pr(R, C), X = x.segments[x.segments.length - 1], B = {
              p0: R,
              p1: C,
              t0: F,
              t1: z,
            ...
            x.segments.push(B), x.length += I;
          }
          for (var b = 0; b < h.length; b++) {
            var E = h[b], M = h[b - 1];
            M && (E.startDist = M.startDist + M.length), m(E, E.p0, y[b * g], 0, n.bezierProjPcts[0]);
            for (var A = 0; A < g - 1; A++)
            ...
            switch (a.edgeType) {
              ...
              case "multibezier": {
                for (var p = l(), y, g = 0, m = 0, b = 0; b < p.length; b++) {
                  for (var E = p[d ? b : p.length - 1 - b], M = 0; M < E.segments.length; M++) {
                    var A = E.segments[d ? M : E.segments.length - 1 - M], x = b === p.length - 1 && M === E.segments.length - 1;
                ...
                var R = y.cp, C = y.segment, F = (v - g) / C.length, z = C.t1 - C.t0, I = d ? C.t0 + z * F : C.t1 - z * F;
                I = ka(0, I, 1), e = ea(R.p0, R.p1, R.p2, I), c = dg(R.p0, R.p1, R.p2, I);
                break;
              }
              case "straight":
              case "segments":
              case "haystack": {
                for (var X = 0, B, re, W, Z, ie = a.allpts.length, ue = 0; ue + 3 < ie && (d ? (W = {
                  x: a.allpts[ue],
                  y: a.allpts[ue + 1]
                }, Z = {
                  x: a.allpts[ue + 2],
      ...
      var r = t._private, a = e ? e + "-" : "", n = t.pstyle(a + "label").strValue, i = t.pstyle("text-transform").value, s = function(pe, se) {
        return se ? (xr(r.rscratch, pe, e, se), se) : er(r.rscratch, pe, e);
      };
      if (!n)
        return "";
...
`), h = t.pstyle("text-max-width").pfValue, c = t.pstyle("text-overflow-wrap").value, d = c === "anywhere", v = [], p = /[\s\u200b]+/, y = d ? "" : " ", g = 0; g < f.length; g++) {
          ...
          if (E > h) {
            for (var A = m.split(p), x = "", R = 0; R < A.length; R++) {
              var C = A[R], F = x.length === 0 ? C : x + y + C, z = this.calculateLabelDimensions(t, F), I = z.width;
              I <= h ? x += C + y : (x && v.push(x), x = C + y);
            }
            x.match(/^[\s\u200b]+$/) || v.push(x);
          } else
            v.push(m);
...
`)), s("labelWrapKey", l);
        ...
        var X = t.pstyle("text-max-width").pfValue, B = "", re = "…", W = !1;
...
`), g = 0; g < y.length; g++) {
        var m = y[g], b = c.measureText(m), E = Math.ceil(b.width), M = l;
        v = Math.max(E, v), p += M;
      }
      return v += s, p += s, n[a] = {
        width: v,
    ...
    ya.registerBinding = function(t, e, r, a) {
      ...
      t.registerBinding(t.container, "mousedown", function(N) {
        if (R(N)) {
          N.preventDefault(), g(), t.hoverData.capture = !0, t.hoverData.which = N.which;
          ...
          if (N.which == 3) {
            ...
            Le && Le.activate();
            {
              if (Le != null && t.nodeIsGrabbable(Le)) {
                ...
                if (u(Le), !Le.selected())
                  ze = t.dragData.possibleDragElements = le.collection(), v(Le, {
                    addToList: ze
            ...
            if (Ze && Ze.pannable() && Ze.active() && Ze.unactivate(), (!Ze || !Ze.grabbed()) && Qe != ut && (ut && r(ut, ["mouseout", "tapdragout"], N, {
              ...
              if (It) {
                if (me.boxSelectionEnabled() && br)
                  Ze && Ze.grabbed() && (p(Ct), Ze.emit("freeon"), Ct.emit("free"), t.dragData.didDrag && (Ze.emit("dragfreeon"), Ct.emit("dragfree"))), xa();
                else if (Ze && Ze.grabbed() && t.nodeIsDraggable(Ze)) {
                  var Ht = !t.dragData.didDrag;
                  Ht && t.redrawHint("eles", !0), t.dragData.didDrag = !0, t.hoverData.draggingEles || d(Ct, {
                    inDragLayer: !0
                  });
                  var Gt = {
                    x: 0,
        ...
        if (le) {
          ...
          if (t.data.bgActivePosistion && (t.redrawHint("select", !0), t.redraw()), t.hoverData.tapholdCancelled = !0, t.data.bgActivePosistion = void 0, ze && ze.unactivate(), t.hoverData.which === 3) {
            ...
            !t.hoverData.isOverThresholdDrag && (r(ze, ["click", "tap", "vclick"], N, {
              x: me[0],
              y: me[1]
            }), F = !1, N.timeStamp - z <= De.multiClickDebounceTime() ? (C && clearTimeout(C), F = !0, z = null, r(ze, ["dblclick", "dbltap", "vdblclick"], N, {
              x: me[0],
              y: me[1]
            })) : (C = setTimeout(function() {
              F || r(ze, ["oneclick", "onetap", "voneclick"], N, {
                x: me[0],
                y: me[1]
              });
            }, De.multiClickDebounceTime()), z = N.timeStamp)), ze == null && !t.dragData.didDrag && !t.hoverData.selecting && !t.hoverData.dragged && !a(N) && (De.$(e).unselect(["tapunselect"]), Le.length > 0 && t.redrawHint("eles", !0), t.dragData.possibleDragElements = Le = De.collection()), Ke == ze && !t.dragData.didDrag && !t.hoverData.selecting && Ke != null && Ke._private.selectable && (t.hoverData.dragging || (De.selectionType() === "additive" || at ? Ke.selected() ? Ke.unselect(["tapunselect"]) : Ke.select(["tapselect"]) : at || (De.$(e).unmerge(Ke).unselect(["tapunselect"]), Ke.select(["tapselect"]))), t.redrawHint("eles", !0)), t.hoverData.selecting) {
            ...
            if (t.hoverData.dragging && (t.hoverData.dragging = !1, t.redrawHint("select", !0), t.redrawHint("eles", !0), t.redraw()), !be[4]) {
              ...
              p(Le), Je && (ze.emit("freeon"), Le.emit("free"), t.dragData.didDrag && (ze.emit("dragfreeon"), Le.emit("dragfree")));
            }
          }
      ...
      t.registerBinding(t.container, "wheel", I, !0), t.registerBinding(window, "scroll", function(N) {
      ...
      var X, B, re, W, Z, ie, ue, pe, se, de, we, Te, Ee, ye = function(N, le, De, me) {
      ...
      t.registerBinding(t.container, "touchstart", xe = function(N) {
        if (t.hasTouchStarted = !0, !!R(N)) {
          g(), t.touchData.capture = !0, t.data.bgActivePosistion = void 0;
          ...
          if (N.touches[1]) {
            ...
            se = Ke[0], de = Ke[1], we = Ke[2], Te = Ke[3], X = N.touches[0].clientX - se, B = N.touches[0].clientY - de, re = N.touches[1].clientX - se, W = N.touches[1].clientY - de, Ee = 0 <= X && X <= we && 0 <= re && re <= we && 0 <= B && B <= Te && 0 <= W && W <= Te;
            var Le = le.pan(), ze = le.zoom();
            Z = ye(X, B, re, W), ie = ae(X, B, re, W), ue = [(X + re) / 2, (B + W) / 2], pe = [(ue[0] - Le.x) / ze, (ue[1] - Le.y) / ze];
            var at = 200, dt = at * at;
      ...
      t.registerBinding(window, "touchmove", Ce = function(N) {
        ...
        if (!(!le && !R(N))) {
          ...
          if (le && t.touchData.cxt) {
            ...
            var cr = t.touchData.start, nr = N.touches[0].clientX - se, Rt = N.touches[0].clientY - de, Qt = N.touches[1].clientX - se, _t = N.touches[1].clientY - de, Fu = ye(nr, Rt, Qt, _t), lp = Fu / Z;
            if (Ee) {
              var fp = nr - X, hp = Rt - B, cp = Qt - re, vp = _t - W, dp = (fp + cp) / 2, gp = (hp + vp) / 2, rn = me.zoom(), qi = rn * lp, jn = me.pan(), zu = pe[0] * rn + jn.x, Gu = pe[1] * rn + jn.y, pp = {
                x: -qi / rn * (zu - jn.x - dp) + zu,
                y: -qi / rn * (Gu - jn.y - gp) + Gu
              };
              if (cr && cr.active()) {
                var Gt = t.dragData.touchDragEles;
                p(Gt), t.redrawHint("drag", !0), t.redrawHint("eles", !0), cr.unactivate().emit("freeon"), Gt.emit("free"), t.dragData.didDrag && (cr.emit("dragfreeon"), Gt.emit("dragfree"));
              }
              me.viewport({
                zoom: qi,
                pan: pp,
                cancelOnFailedZoom: !0
              }), me.emit("pinchzoom"), Z = Fu, X = nr, B = Rt, re = Qt, W = _t, t.pinching = !0;
            }
            if (N.touches[0]) {
              var ze = t.projectIntoViewport(N.touches[0].clientX, N.touches[0].clientY);
              be[0] = ze[0], be[1] = ze[1];
          ...
          if (!N.touches[0]) {
            if (!N.touches[0]) {
              ...
              if (le != null) {
                ...
                p(Je), t.redrawHint("drag", !0), t.redrawHint("eles", !0), Ct && (le.emit("freeon"), Je.emit("free"), t.dragData.didDrag && (le.emit("dragfreeon"), Je.emit("dragfree"))), r(le, ["touchend", "tapend", "vmouseup", "tapdragout"], N, {
                  x: Le[0],
                  y: Le[1]
    ...
    yr.generatePolygon = function(t, e) {
      ...
      return this.nodeShapes[t] = {
        ...
        intersectLine: function(f, h, c, d, v, p, y) {
          return Bf(v, p, this.points, f, h, c, d);
        },
        checkPoint: function(f, h, c, d, v, p, y) {
          return kf(f, h, this.points, p, y, d, v);
        }
      };
      ...
      return this.nodeShapes.barrel = {
        ...
        generateBarrelBezierPts: function(e, r, a, n) {
          var i = r / 2, s = e / 2, o = a - s, l = a + s, u = n - i, f = n + i, h = gi(e, r), c = h.heightOffset, d = h.widthOffset, v = h.ctrlPtOffsetPct * e, p = {
        ...
        checkPoint: function(e, r, a, n, i, s, o) {
          ...
          if (gr(e, r, this.points, s, o, n, i - 2 * u, [0, -1], a) || gr(e, r, this.points, s, o, n - 2 * f, i, [0, -1], a))
            return !0;
          for (var h = this.generateBarrelBezierPts(n, i, s, o), c = function(x, R, C) {
            var F = C[4], z = C[2], I = C[0], X = C[5], B = C[1], re = Math.min(F, I), W = Math.max(F, I), Z = Math.min(X, B), ie = Math.max(X, B);
            if (re <= x && x <= W && Z <= R && R <= ie) {
              var ue = Ff(F, z, I), pe = Nf(ue[0], ue[1], ue[2], x), se = pe.filter(function(de) {
                return 0 <= de && de <= 1;
              });
              if (se.length > 0)
                return se[0];
            ...
            if (g != null) {
              var m = y[5], b = y[3], E = y[1], M = St(m, b, E, g);
    ...
    ma.clientFunctions = ["redrawHint", "render", "renderTo", "matchCanvasSize", "nodeShapeImpl", "arrowShapeImpl"], ma.init = function(t) {
      ...
      if (D) {
        ...
        if (r.className.indexOf(s) < 0 && (r.className = (r.className || "") + " " + s), !o) {
          ...
          l.id = i, l.textContent = "." + s + " { position: relative; }", n.insertBefore(l, n.children[0]);
        }
        ...
        return function() {
          ...
          if (!a.dequeueingSetup) {
            ...
            var i = ln(function() {
              ...
              var h = dr(), c = n.averageRedrawTime, d = n.lastRedrawTime, v = [], p = n.cy.extent(), y = n.getPixelRatio();
              for (u || n.flushRenderedStyleQueue(); ; ) {
                var g = dr(), m = g - h, b = g - f;
                if (d < $i) {
                  var E = $i - (u ? c : 0);
    ...
    }(), pu = 25, Wn = 50, Kn = -4, Vi = 3, yg = 7.99, mg = 8, bg = 1024, Eg = 1024, wg = 1024, xg = 0.2, Tg = 0.8, Cg = 10, Dg = 0.15, Sg = 0.1, Lg = 0.9, Ag = 0.9, Og = 100, Ng = 1, ba = {
      dequeue: "dequeue",
      downscale: "downscale",
    ...
    Dt.reasons = ba, Dt.getTextureQueue = function(t) {
      ...
      if (d && d.invalidated && (d.invalidated = !1, d.texture.invalidatedWidth -= d.width), d)
        return d;
      var v;
      if (f <= pu ? v = pu : f <= Wn ? v = Wn : v = Math.ceil(f / Wn) * Wn, f > wg || h > Eg)
        return null;
      ...
      }, b = n && n === ba.dequeue, E = n && n === ba.highQuality, M = n && n === ba.downscale, A, x = a + 1; x <= Vi; x++) {
        var R = l.get(t, x);
        if (R) {
          A = R;
          break;
      ...
      if (y.context.setTransform(1, 0, 0, 1, 0, 0), y.context.clearRect(y.usedWidth, 0, h, v), m(C))
        ...
        if (E) {
          for (var z = A.level; z > a; z--)
            C = i.getElement(t, e, r, z, ba.downscale);
          F();
        } else
      ...
      return d = {
        ...
        height: f,
        scaledLabelShown: c
      }, y.usedWidth += Math.ceil(h + mg), y.eleCaches.push(d), l.set(t, a, d), i.checkTextureFullness(y), d;
    ...
    var Ig = 1, en = -4, Zn = 2, Rg = 3.99, kg = 50, Pg = 50, Bg = 0.15, Fg = 0.1, zg = 0.9, Gg = 0.9, $g = 1, yu = 250, Vg = 4e3 * 4e3, _g = !0, mu = function(e) {
    ...
    }, Mt = mu.prototype, bu = 0, Ug = Math.pow(2, 53) - 1;
    Mt.makeLayer = function(t, e) {
      ...
      var l = a.layersByLevel, u = Math.pow(2, r), f = l[r] = l[r] || [], h, c = a.levelIsComplete(r, t), d, v = function() {
        var F = function(re) {
          ...
          if (!d)
            for (var W = r + re; en <= W && W <= Zn && !F(W); W += re)
              ;
        };
        z(1), z(-1);
      ...
      for (var g = null, m = t.length / Ig, b = !o, E = 0; E < t.length; E++) {
        var M = t[E], A = M._private.rscratch, x = A.imgLayerCaches = A.imgLayerCaches || {}, R = x[r];
        if (R) {
          g = R;
          continue;
        }
        if ((!g || g.eles.length >= m || !xs(g.bb, M.boundingBox())) && (g = y({
          insert: !0,
          after: g
        }), !g))
          return null;
    ...
    hr.drawElement = function(t, e, r, a, n, i) {
      ...
      if (!(u.w === 0 || u.h === 0)) {
        ...
        if (f != null) {
          ...
          if (h === 0)
            return;
          var c = s(l, e), d = u.x1, v = u.y1, p = u.w, y = u.h, g, m, b, E, M;
          if (c !== 0) {
            ...
            g = d, m = v;
          var R;
          h !== 1 && (R = t.globalAlpha, t.globalAlpha = R * h), t.drawImage(f.texture.canvas, f.x, 0, f.width, f.height, g, m, p, y), h !== 1 && (t.globalAlpha = R), c !== 0 && (t.rotate(-c), t.translate(-b, -E), M || l.setImgSmoothing(t, !1));
        } else
          r.drawElement(t, e);
      }
    };
    ...
    hr.drawCachedElement = function(t, e, r, a, n, i) {
      ...
      if (!(c.w === 0 || c.h === 0 || !e.visible()) && (!a || hi(c, a))) {
        ...
        s.drawElementUnderlay(t, e), s.drawCachedElementPortion(t, e, l, r, n, d, Wg, Jg), (!v || !p) && s.drawCachedElementPortion(t, e, u, r, n, d, Kg, _i), v && !p && (s.drawCachedElementPortion(t, e, f, r, n, d, Zg, _i), s.drawCachedElementPortion(t, e, h, r, n, d, Qg, _i)), s.drawElementOverlay(t, e);
      }
    ...
    mr.drawEdge = function(t, e, r) {
      ...
      if (!(i && !e.visible()) && !(o.badLine || o.allpts == null || isNaN(o.allpts[0]))) {
        ...
        var u = i ? e.pstyle("opacity").value : 1, f = i ? e.pstyle("line-opacity").value : 1, h = e.pstyle("curve-style").value, c = e.pstyle("line-style").value, d = e.pstyle("width").pfValue, v = e.pstyle("line-cap").value, p = u * f, y = u * f, g = function() {
          ...
          h === "straight-triangle" ? (s.eleStrokeStyle(t, e, I), s.drawEdgeTrianglePath(e, t, o.allpts)) : (t.lineWidth = d, t.lineCap = v, s.eleStrokeStyle(t, e, I), s.drawEdgePath(e, t, o.allpts, c), t.lineCap = "butt");
        }, m = function() {
        ...
        if (A) {
          var x = e.pstyle("ghost-offset-x").pfValue, R = e.pstyle("ghost-offset-y").pfValue, C = e.pstyle("ghost-opacity").value, F = p * C;
          t.translate(x, R), g(F), E(F), t.translate(-x, -R);
        }
        b(), g(), E(), m(), M(), r && t.translate(l.x1, l.y1);
      }
    };
    var Tu = function(e) {
    ...
    mr.drawEdgeOverlay = Tu("overlay"), mr.drawEdgeUnderlay = Tu("underlay"), mr.drawEdgePath = function(t, e, r, a) {
      ...
      if (u) {
        var g = l.arrowPathCache = l.arrowPathCache || [], m = kr(n), b = g[m];
    ...
    Ui.safeDrawImage = function(t, e, r, a, n, i, s, o, l, u) {
      ...
      if (f(r, "background-width", "value", a) !== "auto" && (f(r, "background-width", "units", a) === "%" ? C = f(r, "background-width", "pfValue", a) * y : C = f(r, "background-width", "pfValue", a)), f(r, "background-height", "value", a) !== "auto" && (f(r, "background-height", "units", a) === "%" ? F = f(r, "background-height", "pfValue", a) * g : F = f(r, "background-height", "pfValue", a)), !(C === 0 || F === 0)) {
        ...
        var I = o - y / 2, X = f(r, "background-position-x", "units", a), B = f(r, "background-position-x", "pfValue", a);
        X === "%" ? I += (y - C) * B : I += B;
        ...
        ie === "%" ? Z += (g - F) * ue : Z += ue;
        var pe = f(r, "background-offset-y", "units", a), se = f(r, "background-offset-y", "pfValue", a);
        pe === "%" ? Z += (g - F) * se : Z += se, m.pathCache && (I -= o, Z -= l, o = 0, l = 0);
        ...
        if (A === "no" && we ? (i.setImgSmoothing(t, !1), Te = !0) : A === "yes" && !we && (i.setImgSmoothing(t, !0), Te = !0), c === "no-repeat")
          E && (t.save(), m.pathCache ? t.clip(m.pathCache) : (i.nodeShapes[i.getNodeShape(r)].draw(t, o, l, y, g), t.clip())), i.safeDrawImage(t, e, 0, 0, x, R, I, Z, C, F), E && t.restore();
        else {
          var Ee = t.createPattern(e, c);
          t.fillStyle = Ee, i.nodeShapes[i.getNodeShape(r)].draw(t, o, l, y, g), t.translate(I, Z), t.fill(), t.translate(-I, -Z);
        }
    ...
    _r.getTextAngle = function(t, e) {
      ...
      if (!(n && (o === 0 || e.pstyle("text-opacity").value === 0))) {
        ...
        if (c != null && c !== "" && !isNaN(l) && !isNaN(u)) {
          ...
          var d = r ? r + "-" : "", v = er(s, "labelWidth", r), p = er(s, "labelHeight", r), y = e.pstyle(d + "text-margin-x").pfValue, g = e.pstyle(d + "text-margin-y").pfValue, m = e.isEdge(), b = e.pstyle("text-halign").value, E = e.pstyle("text-valign").value;
          ...
          var A = e.pstyle("text-background-opacity").value, x = e.pstyle("text-border-opacity").value, R = e.pstyle("text-border-width").pfValue, C = e.pstyle("text-background-padding").pfValue;
          if (A > 0 || R > 0 && x > 0) {
            var F = l - C;
            switch (b) {
              case "left":
            ...
            if (A > 0) {
              ...
              W.indexOf("round") === 0 ? jg(t, F, z, I, X, 2) : t.fillRect(F, z, I, X), t.fillStyle = B;
            }
            if (R > 0 && x > 0) {
              var Z = t.strokeStyle, ie = t.lineWidth, ue = e.pstyle("text-border-color").value, pe = e.pstyle("text-border-style").value;
              if (t.strokeStyle = "rgba(" + ue[0] + "," + ue[1] + "," + ue[2] + "," + x * o + ")", t.lineWidth = R, t.setLineDash)
                switch (pe) {
                  case "dotted":
                    t.setLineDash([1, 1]);
                    break;
                  case "dashed":
                    ...
                    break;
                  case "double":
                    t.lineWidth = R / 4, t.setLineDash([]);
                    break;
                  case "solid":
                    t.setLineDash([]);
                    break;
                }
              if (t.strokeRect(F, z, I, X), pe === "double") {
                var se = R / 2;
                t.strokeRect(F + se, z + se, I - se * 2, X - se * 2);
              }
          ...
          if (de > 0 && (t.lineWidth = de), e.pstyle("text-wrap").value === "wrap") {
            var we = er(s, "labelWrapCachedLines", r), Te = er(s, "labelLineHeight", r), Ee = v / 2, ye = this.getLabelJustification(e);
            switch (ye === "auto" || (b === "left" ? ye === "left" ? l += -v : ye === "center" && (l += -Ee) : b === "center" ? ye === "left" ? l += -Ee : ye === "right" && (l += Ee) : b === "right" && (ye === "center" ? l += Ee : ye === "right" && (l += v))), E) {
              case "top":
                u -= (we.length - 1) * Te;
                break;
              case "center":
    ...
    Ea.drawNode = function(t, e, r) {
      ...
      if (!(!k(h.x) || !k(h.y)) && !(i && !e.visible())) {
        ...
        for (var m = e.pstyle("background-image"), b = m.value, E = new Array(b.length), M = new Array(b.length), A = 0, x = 0; x < b.length; x++) {
          var R = b[x], C = E[x] = R != null && R !== "none";
          if (C) {
            var F = e.cy().style().getIndexedStyle(e, "background-image-crossorigin", "value", x);
            A++, M[x] = s.getCachedImage(R, F, function() {
              u.backgroundTimestamp = Date.now(), e.emitAndNotify("background");
            });
          }
        }
        var z = e.pstyle("background-blacken").value, I = e.pstyle("border-width").pfValue, X = e.pstyle("background-opacity").value * c, B = e.pstyle("border-color").value, re = e.pstyle("border-style").value, W = e.pstyle("border-opacity").value * c;
        t.lineJoin = "miter";
        var Z = function() {
        ...
        }, ue = e.pstyle("shape").strValue, pe = e.pstyle("shape-polygon-points").pfValue;
        if (d) {
          t.translate(h.x, h.y);
          var se = s.nodePathCache = s.nodePathCache || [], de = hs(ue === "polygon" ? ue + "," + pe.join(",") : ue, "" + l, "" + o), we = se[de];
          we != null ? (v = we, p = !0, f.pathCache = v) : (v = new Path2D(), se[de] = f.pathCache = v);
        }
        var Te = function() {
          if (!p) {
            var ke = h;
          ...
          s.hasPie(e) && (s.drawPie(t, e, Ge), ke && (d || s.nodeShapes[s.getNodeShape(e)].draw(t, h.x, h.y, o, l)));
        }, ae = function() {
          var ke = arguments.length > 0 && arguments[0] !== void 0 ? arguments[0] : c, Ge = (z > 0 ? z : -z) * ke, et = z > 0 ? 0 : 255;
          z !== 0 && (s.colorFillStyle(t, et, et, et, Ge), d ? t.fill(v) : t.fill());
        }, xe = function() {
          if (I > 0) {
            if (t.lineWidth = I, t.lineCap = "butt", t.setLineDash)
              switch (re) {
                case "dotted":
                  t.setLineDash([1, 1]);
                  break;
        ...
        if (He) {
          ...
          t.translate(qe, Re), Z(Me * X), Te(), Ee(Ve, !0), ie(Me * W), xe(), ye(z !== 0 || I !== 0), Ee(Ve, !1), ae(Ve), t.translate(-qe, -Re);
        }
        d && t.translate(-h.x, -h.y), Oe(), d && t.translate(h.x, h.y), Z(), Te(), Ee(c, !0), ie(), xe(), ye(z !== 0 || I !== 0), Ee(c, !1), ae(), d && t.translate(-h.x, -h.y), Ie(), Ce(), r && t.translate(g.x1, g.y1);
      }
    };
    var Cu = function(e) {
      if (!["overlay", "underlay"].includes(e))
    ...
    Wt.getPixelRatio = function() {
      ...
      if (a === "radial-gradient")
        if (r.isEdge()) {
          var u = r.sourceEndpoint(), f = r.targetEndpoint(), h = r.midpoint(), c = Pr(u, h), d = Pr(f, h);
        ...
        var b = s ? {
          x: 0,
          y: 0
        } : r.position(), E = r.paddedWidth(), M = r.paddedHeight(), A = E / 2, x = M / 2, R = r.pstyle("background-gradient-direction").value;
        switch (R) {
          case "to-bottom":
      ...
      }, R = s.prevViewport, C = R === void 0 || x.zoom !== R.zoom || x.pan.x !== R.pan.x || x.pan.y !== R.pan.y;
      !C && !(p && !v) && (s.motionBlurPxRatio = 1), i && (A = i), E *= o, A.x *= o, A.y *= o;
      var F = s.getCachedZSortedEles();
      function z(Re, Me, Ve, Fe, ke) {
      ...
      function I(Re, Me) {
        ...
        !s.clearingMotionBlur && (Re === u.bufferContexts[s.MOTIONBLUR_BUFFER_NODE] || Re === u.bufferContexts[s.MOTIONBLUR_BUFFER_DRAG]) ? (Ve = {
          x: M.x * d,
          y: M.y * d
        }, Fe = b * d, ke = s.canvasWidth * d, Ge = s.canvasHeight * d) : (Ve = A, Fe = E, ke = s.canvasWidth, Ge = s.canvasHeight), Re.setTransform(1, 0, 0, 1, 0, 0), Me === "motionBlur" ? z(Re, 0, 0, ke, Ge) : !e && (Me === void 0 || Me) && Re.clearRect(0, 0, ke, Ge), r || (Re.translate(Ve.x, Ve.y), Re.scale(Fe, Fe)), i && Re.translate(i.x, i.y), n && Re.scale(n, n);
      }
      if (h || (s.textureDrawLastFrame = !1), h) {
        ...
        s.colorFillStyle(B, W[0], W[1], W[2], Z), B.fillRect(0, 0, x.width, x.height);
        var b = l.zoom();
        I(B, !1), B.clearRect(x.mpan.x, x.mpan.y, x.width / x.zoom / o, x.height / x.zoom / o), B.drawImage(re, x.mpan.x, x.mpan.y, x.width / x.zoom / o, x.height / x.zoom / o);
      } else
        s.textureOnViewport && !e && (s.textureCache = null);
      var ie = l.extent(), ue = s.pinching || s.hoverData.dragging || s.swipePanning || s.data.wheelZooming || s.hoverData.draggingEles || s.cy.animated(), pe = s.hideEdgesOnViewport && ue, se = [];
      if (se[s.NODE] = !f[s.NODE] && c && !s.clearedForMotionBlur[s.NODE] || s.clearingMotionBlur, se[s.NODE] && (s.clearedForMotionBlur[s.NODE] = !0), se[s.DRAG] = !f[s.DRAG] && c && !s.clearedForMotionBlur[s.DRAG] || s.clearingMotionBlur, se[s.DRAG] && (s.clearedForMotionBlur[s.DRAG] = !0), f[s.NODE] || r || a || se[s.NODE]) {
        var de = c && !se[s.NODE] && d !== 1, B = e || (de ? s.data.bufferContexts[s.MOTIONBLUR_BUFFER_NODE] : u.contexts[s.NODE]), we = c && !de ? "motionBlur" : void 0;
        I(B, we), pe ? s.drawCachedNodes(B, F.nondrag, o, ie) : s.drawLayeredElements(B, F.nondrag, o, ie), s.debug && s.drawDebugPoints(B, F.nondrag), !r && !c && (f[s.NODE] = !1);
      }
      if (!a && (f[s.DRAG] || r || se[s.DRAG])) {
        var de = c && !se[s.DRAG] && d !== 1, B = e || (de ? s.data.bufferContexts[s.MOTIONBLUR_BUFFER_DRAG] : u.contexts[s.DRAG]);
        I(B, c && !de ? "motionBlur" : void 0), pe ? s.drawCachedNodes(B, F.drag, o, ie) : s.drawCachedElements(B, F.drag, o, ie), s.debug && s.drawDebugPoints(B, F.drag), !r && !c && (f[s.DRAG] = !1);
      }
      if (s.showFps || !a && f[s.SELECT_BOX] && !r) {
        var B = e || u.contexts[s.SELECT_BOX];
        if (I(B), s.selection[4] == 1 && (s.hoverData.selecting || s.touchData.selecting)) {
        ...
        if (u.bgActivePosistion && !s.hoverData.selecting) {
          ...
          B.fillStyle = "rgba(" + m.core("active-bg-color").value[0] + "," + m.core("active-bg-color").value[1] + "," + m.core("active-bg-color").value[2] + "," + m.core("active-bg-opacity").value + ")", B.beginPath(), B.arc(Ee.x, Ee.y, m.core("active-bg-size").pfValue / b, 0, 2 * Math.PI), B.fill();
        }
        var ye = s.lastRedrawTime;
        if (s.showFps && ye) {
          ye = Math.round(ye);
          var ae = Math.round(1e3 / ye);
          B.setTransform(1, 0, 0, 1, 0, 0), B.fillStyle = "rgba(255, 0, 0, 0.75)", B.strokeStyle = "rgba(255, 0, 0, 0.75)", B.lineWidth = 1, B.fillText("1 frame = " + ye + " ms = " + ae + " fps", 0, 20);
          var xe = 60;
      ...
      if (c && d !== 1) {
        var Ce = u.contexts[s.NODE], Oe = s.data.bufferCanvases[s.MOTIONBLUR_BUFFER_NODE], Ie = u.contexts[s.DRAG], He = s.data.bufferCanvases[s.MOTIONBLUR_BUFFER_DRAG], qe = function(Me, Ve, Fe) {
          Me.setTransform(1, 0, 0, 1, 0, 0), Fe || !g ? Me.clearRect(0, 0, s.canvasWidth, s.canvasHeight) : z(Me, 0, 0, s.canvasWidth, s.canvasHeight);
          var ke = d;
          Me.drawImage(
            Ve,
            // img
    ...
    Mr.drawPolygonPath = function(t, e, r, a, n, i) {
      ...
      var i = a / 2, s = n / 2, o = e - i, l = e + i, u = r - s, f = r + s, h = gi(a, n), c = h.widthOffset, d = h.heightOffset, v = h.ctrlPtOffsetPct * c;
    ...
    function Mu(t) {
      ...
      var u = Ut(), f = function(ae) {
        ...
        return z("", I(ae, "labelX", "labelY"), ae);
      }, B = function(ae) {
        return z("source", I(ae, "sourceLabelX", "sourceLabelY"), ae);
      }, re = function(ae) {
        return z("target", I(ae, "targetLabelX", "targetLabelY"), ae);
      }, W = function(ae) {
        return h(M(ae));
      }, Z = function(ae) {
        return h(x(ae));
        ...
        return Ce;
      }, pe = e.data.eleTxrCache = new ja(e, {
        getKey: d,
      ...
      e.onUpdateEleCalcs(function(ae, xe) {
        pe.invalidateElements(xe), se.invalidateElements(xe), de.invalidateElements(xe), we.invalidateElements(xe), Te.invalidateElements(xe);
      ...
      pe.onDequeue(Ee), se.onDequeue(Ee), de.onDequeue(Ee), we.onDequeue(Ee);
    }
    ...
    function ku(t, e, r) {
      var a = r, n = function(R) {
        ft("Can not register `" + e + "` for `" + t + "` since `" + R + "` already exists in the prototype and can not be overridden");
      };
      if (t === "core") {
        if (Za.prototype[e])
          return n(e);
        ...
        for (var i = function(R) {
          this.options = R, r.call(this, R), L(this._private) || (this._private = {}), this._private.cy = R.cy, this._private.listeners = [], this.createEmitter();
        ...
        var h = function(R) {
          return R._private.cy;
        }, c = {
          addEventFields: function(R, C) {
            C.layout = R, C.cy = h(R), C.target = R;
          },
          bubble: function() {
            return !0;
          },
        ...
        Ue(s, {
          ...
          on: function(R, C) {
            return this.emitter().on(R, C), this;
          },
          one: function(R, C) {
            return this.emitter().one(R, C), this;
          },
          once: function(R, C) {
            return this.emitter().one(R, C), this;
          },
          removeListener: function(R, C) {
            return this.emitter().removeListener(R, C), this;
          },
          removeAllListeners: function() {
            return this.emitter().removeAllListeners(), this;
          },
          emit: function(R, C) {
            return this.emitter().emit(R, C), this;
          }
        }), ht.eventAliasesOn(s), a = i;
      } else if (t === "renderer" && e !== "null" && e !== "base") {
        var d = Pu("renderer", "base"), v = d.prototype, p = r, y = r.prototype, g = function() {
          d.apply(this, arguments), p.apply(this, arguments);
        }, m = g.prototype;
        for (var b in v) {
          var E = v[b], M = y[b] != null;
          if (M)
            return n(b);
          m[b] = E;
        }
    ...
    Za.prototype.extension = Xi, ip.forEach(function(t) {
    ...
    return Yr.use = function(t) {
      ...
      return gs(t);
    }, Yr.version = up, Yr.stylesheet = Yr.Stylesheet = Bu, Yr;
  });
})(Dp);
const Uu = Ki;
var Zi = {}, Sp = {
        ...
        function(ee) {
          ...
          function(ee, ve, H) {
            ...
            V.prototype.getNodes = function() {
              ...
              for (var ne = T.MAX_VALUE, oe = -T.MAX_VALUE, J = T.MAX_VALUE, j = -T.MAX_VALUE, Y, te, L, $, k, q = this.nodes, ce = q.length, ge = 0; ge < ce; ge++) {
                var Ae = q[ge];
              ...
              for (var ne = T.MAX_VALUE, oe = -T.MAX_VALUE, J = T.MAX_VALUE, j = -T.MAX_VALUE, Y, te, L, $, k = Q.length, q = 0; q < k; q++) {
                var ce = Q[q];
                Y = ce.getLeft(), te = ce.getRight(), L = ce.getTop(), $ = ce.getBottom(), ne > Y && (ne = Y), oe < te && (oe = te), J > L && (J = L), j < $ && (j = $);
              }
              var ge = new P(ne, J, oe - ne, j - J);
              return ge;
              ...
              for (te.forEach(function(ge) {
                ne.push(ge), oe.add(ge);
              }); ne.length !== 0; ) {
                J = ne.shift(), j = J.getEdges();
                for (var L = j.length, $ = 0; $ < L; $++) {
                  var k = j[$];
                  if (Y = k.getOtherEndInGraph(J, this), Y != null && !oe.has(Y)) {
          ...
          function(ee, ve, H) {
            ...
            T.calcSeparationAmount = function(w, S, G, U) {
              ...
              var V = w.getX(), _ = w.getY(), Q = w.getRight(), ne = w.getX(), oe = w.getBottom(), J = w.getRight(), j = w.getWidthHalf(), Y = w.getHeightHalf(), te = S.getX(), L = S.getY(), $ = S.getRight(), k = S.getX(), q = S.getBottom(), ce = S.getRight(), ge = S.getWidthHalf(), Ae = S.getHeightHalf(), Ne = !1, _e = !1;
              if (U === K) {
                if (P > D)
                ...
                var tt = w.height / w.width, ct = S.height / S.width, Pe = (D - P) / (K - U), $e = void 0, Xe = void 0, rt = void 0, lt = void 0, nt = void 0, je = void 0;
                if (-tt === Pe ? U > K ? (G[0] = ne, G[1] = oe, Ne = !0) : (G[0] = Q, G[1] = _, Ne = !0) : tt === Pe && (U > K ? (G[0] = V, G[1] = _, Ne = !0) : (G[0] = J, G[1] = oe, Ne = !0)), -ct === Pe ? K > U ? (G[2] = k, G[3] = q, _e = !0) : (G[2] = $, G[3] = L, _e = !0) : ct === Pe && (K > U ? (G[2] = te, G[3] = L, _e = !0) : (G[2] = ce, G[3] = q, _e = !0)), Ne && _e)
                  return !1;
                ...
                if (!_e)
                  switch (Xe) {
                    case 1:
                      ...
                      break;
                    case 2:
                      nt = ce, je = D + ge * Pe, G[2] = nt, G[3] = je;
                      break;
                    case 3:
              ...
              var P = w.x, K = w.y, D = S.x, V = S.y, _ = G.x, Q = G.y, ne = U.x, oe = U.y, J = void 0, j = void 0, Y = void 0, te = void 0, L = void 0, $ = void 0, k = void 0, q = void 0, ce = void 0;
              return Y = V - K, L = P - D, k = D * K - P * V, te = oe - Q, $ = _ - ne, q = ne * Q - _ * oe, ce = Y * $ - te * L, ce === 0 ? null : (J = (L * q - $ * k) / ce, j = (te * k - Y * q) / ce, new O(J, j));
          ...
          function(ee, ve, H) {
            ...
            P.prototype.initParameters = function() {
              ...
              var _ = D.getRect(), Q = V.getRect(), ne = new Array(2), oe = new Array(4), J, j, Y, te, L, $, k;
              if (_.intersects(Q)) {
                G.calcSeparationAmount(_, Q, ne, w.DEFAULT_EDGE_LENGTH / 2), $ = 2 * ne[0], k = 2 * ne[1];
      ...
      return (
        ...
        function(ve) {
          ...
          function O(T) {
            ...
            return ve[T].call(w.exports, w, w.exports, O), w.l = !0, w.exports;
          }
          ...
          function(ve, H, O) {
            ...
            for (var S in T)
              w[S] = T[S];
            w.DEFAULT_USE_MULTI_LEVEL_SCALING = !1, w.DEFAULT_RADIAL_SEPARATION = T.DEFAULT_EDGE_LENGTH, w.DEFAULT_COMPONENT_SEPERATION = 60, w.TILE = !0, w.TILING_PADDING_VERTICAL = 10, w.TILING_PADDING_HORIZONTAL = 10, w.TREE_REDUCTION_ON_INCREMENTAL = !1, ve.exports = w;
          },
          /* 2 */
          /***/
          function(ve, H, O) {
          ...
          function(ve, H, O) {
            ...
            Y.prototype.newGraphManager = function() {
              ...
              for (k = 0; k < L.length; k++) {
                var q = L[k];
                if (!$.has(q)) {
                  var ce = q.getSource(), ge = q.getTarget();
                  if (ce == ge)
                    q.getBendpoints().push(new _()), q.getBendpoints().push(new _()), this.createDummyNodesForBendpoints(q), $.add(q);
                  else {
                    var Ae = [];
                    if (Ae = Ae.concat(ce.getEdgeListToNode(ge)), Ae = Ae.concat(ge.getEdgeListToNode(ce)), !$.has(Ae[0])) {
                      if (Ae.length > 1) {
                        var Ne;
              ...
              for (var $ = new V(0, 0), k = Math.ceil(Math.sqrt(L.length)), q = 0, ce = 0, ge = 0, Ae = new _(0, 0), Ne = 0; Ne < L.length; Ne++) {
                Ne % k == 0 && (ge = 0, ce = q, Ne != 0 && (ce += P.DEFAULT_COMPONENT_SEPERATION), q = 0);
                var _e = L[Ne], tt = Q.findCenterOfTree(_e);
                $.x = ge, $.y = ce, Ae = Y.radialLayout(_e, tt, $), Ae.y > q && (q = Math.floor(Ae.y)), ge = Math.floor(Ae.x + P.DEFAULT_COMPONENT_SEPERATION);
              }
              ...
              var ce = J.calculateBounds(L), ge = new j();
              ge.setDeviceOrgX(ce.getMinX()), ge.setDeviceOrgY(ce.getMinY()), ge.setWorldOrgX(k.x), ge.setWorldOrgY(k.y);
            ...
            }, Y.branchRadialLayout = function(L, $, k, q, ce, ge) {
              var Ae = (q - k + 1) / 2;
              Ae < 0 && (Ae += 180);
              var Ne = (Ae + k) % 360, _e = Ne * oe.TWO_PI / 360, tt = ce * Math.cos(_e), ct = ce * Math.sin(_e);
              L.setCenter(tt, ct);
              var Pe = [];
              ...
              for (var Et = Math.abs(q - k) / $e, kt = lt; Xe != $e; kt = ++kt % rt) {
                ...
                if (vt != $) {
                  ...
                  Y.branchRadialLayout(vt, L, vr, qt, ce + ge, ge), Xe++;
                }
              }
              ...
              Object.keys($).forEach(function(k) {
                var q = L.idToDummyNode[k], ce = q.paddingLeft, ge = q.paddingTop;
                L.adjustLocations($[k], q.rect.x, q.rect.y, ce, ge);
              });
              ...
              if (k == L.rows.length) {
                var ge = [];
                L.rows.push(ge), L.rowWidth.push(ce), L.rowHeight.push(0);
              }
              ...
              var Ae;
              L.width - ce >= $ + L.horizontalPadding ? Ae = (L.height + ge) / (ce + $ + L.horizontalPadding) : Ae = (L.height + ge) / L.width, ge = k + L.verticalPadding;
              var Ne;
              ...
              var $ = this.getLongestRowIndex(L), k = L.rowWidth.length - 1, q = L.rows[$], ce = q[q.length - 1], ge = ce.width + L.horizontalPadding;
              if (L.width - L.rowWidth[k] > ge && $ != k) {
                q.splice(-1, 1), L.rows[k].push(ce), L.rowWidth[$] = L.rowWidth[$] - ge, L.rowWidth[k] = L.rowWidth[k] + ge, L.width = L.rowWidth[instance.getLongestRowIndex(L)];
              ...
              for (var L = [], $ = !0, k; $; ) {
                ...
                for (var ge = 0; ge < q.length; ge++)
                  k = q[ge], k.getEdges().length == 1 && !k.getEdges()[0].isInterGraph && k.getChild() == null && (ce.push([k, k.getEdges()[0], k.getOwner()]), $ = !0);
                if ($ == !0) {
              ...
              var ce = k.startX, ge = k.finishX, Ae = k.startY, Ne = k.finishY, _e = 0, tt = 0, ct = 0, Pe = 0, $e = [_e, ct, tt, Pe];
              if (Ae > 0)
                for (var Xe = ce; Xe <= ge; Xe++)
        ...
        function(ve, H, O) {
          ...
          _.prototype.run = function() {
            ...
            for (var q = 0; q < k.length; q++) {
              var ce = k[q], ge = this.idToLNode[ce.data("source")], Ae = this.idToLNode[ce.data("target")];
            ...
            for (var Y = J.length, te = 0; te < Y; te++) {
              ...
              if (L.outerWidth() != null && L.outerHeight() != null ? k = oe.add(new U(j.graphManager, new P(L.position("x") - q.w / 2, L.position("y") - q.h / 2), new K(parseFloat(q.w), parseFloat(q.h)))) : k = oe.add(new U(this.graphManager)), k.id = L.data("id"), k.paddingLeft = parseInt(L.css("padding")), k.paddingTop = parseInt(L.css("padding")), k.paddingRight = parseInt(L.css("padding")), k.paddingBottom = parseInt(L.css("padding")), this.options.nodeDimensionsIncludeLabels && L.isParent()) {
                var ce = L.boundingBox({ includeLabels: !0, includeNodes: !1 }).w, ge = L.boundingBox({ includeLabels: !0, includeNodes: !1 }).h, Ae = L.css("text-halign");
                k.labelWidth = ce, k.labelHeight = ge, k.labelPos = Ae;
              }
...
const Mp = /* @__PURE__ */ mp(Zi);
var Qi = function() {
  var he = function(j, Y, te, L) {
    for (te = te || {}, L = j.length; L--; te[j[L]] = Y)
      ;
    return te;
  }, fe = [1, 4], ee = [1, 13], ve = [1, 12], H = [1, 15], O = [1, 16], T = [1, 20], w = [1, 19], S = [6, 7, 8], G = [1, 26], U = [1, 24], P = [1, 25], K = [6, 7, 11], D = [1, 6, 13, 15, 16, 19, 22], V = [1, 33], _ = [1, 34], Q = [1, 6, 7, 11, 13, 15, 16, 19, 22], ne = {
    trace: function() {
    },
    yy: {},
    ...
    table: [{ 3: 1, 4: 2, 5: 3, 6: [1, 5], 8: fe }, { 1: [3] }, { 1: [2, 1] }, { 4: 6, 6: [1, 7], 7: [1, 8], 8: fe }, { 6: ee, 7: [1, 10], 9: 9, 12: 11, 13: ve, 14: 14, 15: H, 16: O, 17: 17, 18: 18, 19: T, 22: w }, he(S, [2, 3]), { 1: [2, 2] }, he(S, [2, 4]), he(S, [2, 5]), { 1: [2, 6], 6: ee, 12: 21, 13: ve, 14: 14, 15: H, 16: O, 17: 17, 18: 18, 19: T, 22: w }, { 6: ee, 9: 22, 12: 11, 13: ve, 14: 14, 15: H, 16: O, 17: 17, 18: 18, 19: T, 22: w }, { 6: G, 7: U, 10: 23, 11: P }, he(K, [2, 22], { 17: 17, 18: 18, 14: 27, 15: [1, 28], 16: [1, 29], 19: T, 22: w }), he(K, [2, 18]), he(K, [2, 19]), he(K, [2, 20]), he(K, [2, 21]), he(K, [2, 23]), he(K, [2, 24]), he(K, [2, 26], { 19: [1, 30] }), { 20: [1, 31] }, { 6: G, 7: U, 10: 32, 11: P }, { 1: [2, 7], 6: ee, 12: 21, 13: ve, 14: 14, 15: H, 16: O, 17: 17, 18: 18, 19: T, 22: w }, he(D, [2, 14], { 7: V, 11: _ }), he(Q, [2, 8]), he(Q, [2, 9]), he(Q, [2, 10]), he(K, [2, 15]), he(K, [2, 16]), he(K, [2, 17]), { 20: [1, 35] }, { 21: [1, 36] }, he(D, [2, 13], { 7: V, 11: _ }), he(Q, [2, 11]), he(Q, [2, 12]), { 21: [1, 37] }, he(K, [2, 25]), he(K, [2, 27])],
    ...
    parse: function(Y) {
      var te = this, L = [0], $ = [], k = [null], q = [], ce = this.table, ge = "", Ae = 0, Ne = 0, _e = 2, tt = 1, ct = q.slice.call(arguments, 1), Pe = Object.create(this.lexer), $e = { yy: {} };
...
const Rp = () => {
  ...
  for (let fe = Jt.length - 1; fe >= 0; fe--)
    if (Jt[fe].level < he)
      return Jt[fe];
  return null;
}, Pp = () => Jt.length > 0 ? Jt[0] : null, Bp = (he, fe, ee, ve) => {
...
const Gp = (he) => {
  Xu = he;
}, $p = () => qr, Vp = (he) => Jt[he], ji = (he) => Ji[he], _p = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  addNode: Bp,
  clear: Rp,
  decorateNode: zp,
  ...
  setErrorHandler: Gp,
  type2Str: Wr
}, Symbol.toStringTag, { value: "Module" })), qu = 12, Up = function(he, fe, ee) {
  ...
  const ee = fe.width, ve = fe.height, H = 0.15 * ee, O = 0.25 * ee, T = 0.35 * ee, w = 0.2 * ee;
  he.append("path").attr("id", "node-" + fe.id).attr("class", "node-bkg node-" + Wr(fe.type)).attr(
    "d",
    `M0 0 a${H},${H} 0 0,1 ${ee * 0.25},${-1 * ee * 0.1}
      ...
      a${w},${w} 1 0,1 ${-1 * ee * 0.15},${1 * ve * 0.65}
...
}, Qp = function(he, fe, ee, ve) {
  const H = ve.htmlLabels, O = ee % (qu - 1), T = he.append("g");
  fe.section = O;
  let w = "section-" + O;
  O < 0 && (w += " section-root"), T.attr("class", (fe.class ? fe.class + " " : "") + "mindmap-node " + w);
...
`);
  ...
  switch (fe.type) {
    ...
    case yt.RECT:
      ...
      break;
    case yt.CIRCLE:
      S.attr("transform", "translate(" + fe.width / 2 + ", " + +fe.height / 2 + ")"), qp(S, fe);
      break;
    case yt.CLOUD:
      Hp(S, fe);
      break;
  ...
  return Hu(fe.id, T), fe.height;
}, Jp = function(fe, ee, ve, H, O) {
  const T = O % (qu - 1), w = ve.x + ve.width / 2, S = ve.y + ve.height / 2, G = ee.x + ee.width / 2, U = ee.y + ee.height / 2, P = G > w ? w + Math.abs(w - G) / 2 : w - Math.abs(w - G) / 2, K = U > S ? S + Math.abs(S - U) / 2 : S - Math.abs(S - U) / 2, D = G > w ? Math.abs(w - P) / 2 + w : -Math.abs(w - P) / 2 + w, V = U > S ? Math.abs(S - K) / 2 + S : -Math.abs(S - K) / 2 + S;
  fe.append("path").attr(
    "d",
    ve.direction === "TB" || ve.direction === "BT" ? `M${w},${S} Q${w},${V} ${P},${K} T${G},${U}` : `M${w},${S} Q${D},${S} ${P},${K} T${G},${U}`
  ).attr("class", "edge section-edge-" + T + " edge-depth-" + H);
}, jp = function(he) {
  const fe = ji(he.id), ee = he.x || 0, ve = he.y || 0;
  fe.attr("transform", "translate(" + ee + "," + ve + ")");
}, Wu = { drawNode: Qp, positionNode: jp, drawEdge: Jp };
Uu.use(Mp);
function Ku(he, fe, ee, ve) {
  Wu.drawNode(he, fe, ee, ve), fe.children && fe.children.forEach((H, O) => {
...
function ty(he, fe) {
  return new Promise((ee) => {
    const ve = ei("body").append("div").attr("id", "cy").attr("style", "display:none"), H = Uu({
...
function ry(he) {
  he.nodes().map((fe, ee) => {
    const ve = fe.data();
    ve.x = fe.position().x, ve.y = fe.position().y, Wu.positionNode(ve);
...
` + he, ve.parser);
  ...
  const K = await ty(G, H);
  ey(U, K), ry(K), Ep(void 0, S, H.mindmap.padding, H.mindmap.useMaxWidth);
}, ny = {
  draw: ay
}, iy = (he) => {
  let fe = "";
  for (let ee = 0; ee < he.THEME_COLOR_LIMIT; ee++)
    he["lineColor" + ee] = he["lineColor" + ee] || he["cScaleInv" + ee], wp(he["lineColor" + ee]) ? he["lineColor" + ee] = xp(he["lineColor" + ee], 20) : he["lineColor" + ee] = Tp(he["lineColor" + ee], 20);
  for (let ee = 0; ee < he.THEME_COLOR_LIMIT; ee++) {
    const ve = "" + (17 - 3 * ee);
    fe += `

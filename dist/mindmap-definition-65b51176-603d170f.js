import { S as Ca, ao as mp, d as bp, c as nn, l as qr, h as ei, t as Ep, at as wp, au as xp } from "./index-42e57b11.js";
import { c as Tp } from "./createText-1f5f8f92-06704af7.js";
import { i as Cp } from "./is_dark-857914a3.js";
import "vue";
var Ki = {}, Dp = {
  get exports() {
    return Ki;
    ...
    */
    ...
    var Xd = function(e, r, a) {
      ...
      for (var u = 0; u < s.edgeSize; u++) {
        ...
        if (W != Z) {
          for (var ie = qd(F.sourceId, F.targetId, s), oe = s.graphSet[ie], ge = 0, g = s.layoutNodes[z]; oe.indexOf(g.id) === -1; )
...
`), p = 0; p < y.length; p++) {
      ...
      t.registerBinding(window, "touchmove", Ce = function(N) {
        ...
        if (!(!le && !I(N))) {
          ...
          if (le && t.touchData.cxt) {
            ...
            if (Ee) {
              var fp = nr - X, hp = Rt - z, cp = Qt - re, vp = _t - W, dp = (fp + cp) / 2, gp = (hp + vp) / 2, rn = me.zoom(), qi = rn * lp, jn = me.pan(), zu = ge[0] * rn + jn.x, Gu = ge[1] * rn + jn.y, pp = {
    ...
    Dt.reasons = ba, Dt.getTextureQueue = function(t) {
      ...
      if (v && v.invalidated && (v.invalidated = !1, v.texture.invalidatedWidth -= v.width), v)
        return v;
      var d;
      if (f <= pu ? d = pu : f <= Wn ? d = Wn : d = Math.ceil(f / Wn) * Wn, f > wg || h > Eg)
        return null;
    ...
    hr.drawCachedElement = function(t, e, r, a, n, i) {
      ...
      if (!(c.w === 0 || c.h === 0 || !e.visible()) && (!a || hi(c, a))) {
        ...
        s.drawElementUnderlay(t, e), s.drawCachedElementPortion(t, e, l, r, n, v, Wg, Jg), (!d || !g) && s.drawCachedElementPortion(t, e, u, r, n, v, Kg, _i), d && !g && (s.drawCachedElementPortion(t, e, f, r, n, v, Zg, _i), s.drawCachedElementPortion(t, e, h, r, n, v, Qg, _i)), s.drawElementOverlay(t, e);
      }
...
`);
  ...
  const T = O % (qu - 1), x = ve.x + ve.width / 2, S = ve.y + ve.height / 2, G = ee.x + ee.width / 2, U = ee.y + ee.height / 2, P = G > x ? x + Math.abs(x - G) / 2 : x - Math.abs(x - G) / 2, K = U > S ? S + Math.abs(S - U) / 2 : S - Math.abs(S - U) / 2, D = G > x ? Math.abs(x - P) / 2 + x : -Math.abs(x - P) / 2 + x, V = U > S ? Math.abs(S - K) / 2 + S : -Math.abs(S - K) / 2 + S;
  fe.append("path").attr(
    "d",
...
` + he, ve.parser);
  ...
  for (let ee = 0; ee < he.THEME_COLOR_LIMIT; ee++)
    he["lineColor" + ee] = he["lineColor" + ee] || he["cScaleInv" + ee], Cp(he["lineColor" + ee]) ? he["lineColor" + ee] = wp(he["lineColor" + ee], 20) : he["lineColor" + ee] = xp(he["lineColor" + ee], 20);
  for (let ee = 0; ee < he.THEME_COLOR_LIMIT; ee++) {
    const ve = "" + (17 - 3 * ee);
    fe += `
